...
- Always load skills via the `Skill` tool (e.g., `Skill(cafleet)`). At startup, also `Read .claude/agents/web-researcher.md` (resolved from the cafleet repo root) for detailed research methodology — it defines the Discovery Phase, query formulation patterns, synthesis guidance, and output format.
