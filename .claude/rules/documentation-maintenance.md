...
- Update the appropriate `docs/docs/concepts/<page>.md` page (or add a new Concepts page if the feature introduces a new architectural axis)
...
`docs/` is the primary home for all descriptive content — features, architecture, usage, configuration, and project structure land under `docs/` first. `README.md` is a thin entry point (pitch, install commands, docs-site section links) and must be updated only when that surface itself changes — the pitch drifts from `docs/docs/index.md`, the install commands change, or a docs-site section is added or removed. Drift on the thin surface is a blocker for "documentation complete".
