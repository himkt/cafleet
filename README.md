...
- **Member Lifecycle** -- `cafleet member create/delete/list/capture/send-input/exec/safe-exec` commands wrap tmux pane spawning + agent registration into atomic operations; the `agent_placements` table persists the agent-to-pane mapping in the registry. `member send-input` is a safe `tmux send-keys` wrapper for answering an `AskUserQuestion` prompt (digit 1/2/3 or free text) that mirrors the `capture` authorization boundary. Director-side usage is AskUserQuestion-delegated: the Director asks the user via its own `AskUserQuestion` call and then invokes the resolved command via Bash — see [`skills/cafleet/SKILL.md`](skills/cafleet/SKILL.md) "Answer a member's AskUserQuestion prompt" for the canonical three-beat workflow and pane-shapes table
- **Permission-aware Bash Dispatch** -- The Director routes bash via two sibling subcommands, each taking the command as a positional `CMD` argument: `cafleet member safe-exec CMD` is the silent fast-path that re-reads three `settings.json` files on every invocation and matches the inner CMD against the operator's `Bash(...)` allow / deny patterns (allow → exit 0 + dispatch; deny → exit 2 with matched pattern on stderr; ask → exit 3 with searched files + suggested pattern on stderr); `cafleet member exec CMD` is the bare-dispatch fallback (no internal permission check; Claude Code's outer `permissions.ask` on the Director's invocation is the operator-confirmation surface). The Director picks via a check-then-fallback rule: try `safe-exec` first, on exit 3 fall back to `exec`. See [`skills/cafleet/SKILL.md`](skills/cafleet/SKILL.md) "Routing Bash via the Director" for the end-to-end protocol
- **Member Spawn Permission Posture** -- `cafleet member create` always launches the spawned `claude` process with `--permission-mode dontAsk` so the member's Bash tool is enabled and permission prompts auto-resolve silently. Members run cafleet (and any other shell command) directly via the Bash tool. The bash-via-Director protocol (member sends a plain CAFleet message asking the Director to run a command, Director dispatches via `cafleet member safe-exec CMD` or `cafleet member exec CMD`) is the fallback when the harness deny-list rejects a Bash invocation (e.g. `git push`, `rm -rf`); members auto-route in that case after first reconsidering whether the command is correct and necessary. See [`skills/cafleet/SKILL.md`](skills/cafleet/SKILL.md) "Routing Bash via the Director" for details
...
| `cafleet --session-id <id> member send-input --agent-id <id>` | Required | Forward a restricted keystroke (`--choice {1,2,3}` or `--freetext "<text>"`) to a member's pane to answer an AskUserQuestion prompt (Director only); see [docs/spec/cli-options.md](docs/spec/cli-options.md#member-send-input). The workflow is AskUserQuestion-delegated — see [`skills/cafleet/SKILL.md`](skills/cafleet/SKILL.md) "Answer a member's AskUserQuestion prompt" for the canonical three-beat workflow |
| `cafleet --session-id <id> member exec --agent-id <id> --member-id <id> CMD` | Required | Bare bash dispatch (Director only). Keystrokes `! CMD` into the member pane after authorization and pre-flight validation. NO internal permission decision — Claude Code's outer `permissions.ask` on the Director's invocation is the operator-confirmation surface. Use as the smart-routing fallback when `safe-exec` exits 3. See [docs/spec/cli-options.md](docs/spec/cli-options.md#member-exec) and [`skills/cafleet/SKILL.md`](skills/cafleet/SKILL.md) "Routing Bash via the Director" |
| `cafleet --session-id <id> member safe-exec --agent-id <id> --member-id <id> CMD` | Required | Permission-aware bash dispatch (Director only). Re-reads three `settings.json` files on every call, matches the inner CMD against `Bash(...)` allow / deny patterns, and on allow keystrokes `! CMD` into the member pane. Deny exits 2; ask (no allow pattern matches) exits 3 with the searched file list and a suggested pattern. See [docs/spec/cli-options.md](docs/spec/cli-options.md#member-safe-exec) and [`skills/cafleet/SKILL.md`](skills/cafleet/SKILL.md) "Routing Bash via the Director" |

#### `member safe-exec` and `member exec` smart-routing flow

When the bash-via-Director protocol fires (a member's harness denies a Bash invocation and the member auto-routes by sending a plain CAFleet message asking the Director to run a command), the Director picks between the two sibling subcommands using a check-then-fallback rule. **Try `cafleet member safe-exec ... CMD` first.** It decides every dispatch against the union of `Bash(...)` patterns from `<cwd>/.claude/settings.local.json`, `<cwd>/.claude/settings.json`, and `$CLAUDE_CONFIG_DIR/settings.json` (or `~/.claude/settings.json` if unset) — re-read on every call, deny-wins-on-conflict. With `Bash(git status:*)` in the operator's allow list, `cafleet ... member safe-exec ... "git status --short"` keystrokes `! git status --short` into the member's pane and exits 0 — done. With `Bash(git push:*)` in the operator's deny list, `cafleet ... member safe-exec ... "git push origin main"` exits 2 with `Error: command rejected by deny pattern Bash(git push:*) declared in <file>. Offending command: git push origin main` on stderr — relay the rejection to the operator, do not retry. With no matching allow pattern for `cafleet ... member safe-exec ... "rg foo"`, the command exits 3 with the three searched file paths and a suggested `Bash(rg:*)` pattern on stderr — **fall back to `cafleet ... member exec ... "rg foo"`**, which surfaces the operator's outer-layer `permissions.ask` prompt (configured via `Bash(cafleet --session-id * member exec *)` in the operator's `permissions.ask`) and on accept dispatches the command (exit 0). This routes routine pre-rule'd commands through the silent fast-path and surfaces everything else as per-call operator confirmation, so the operator does not have to maintain a comprehensive allow list. `cafleet --json member safe-exec ...` emits a five-key JSON payload (`outcome`, `matched_pattern`, `matched_file`, `offending_substring`, `searched_files`) for all three outcomes; `cafleet --json member exec ...` emits the four-key shape (`member_agent_id`, `pane_id`, `action="bash"`, `value`) since `exec` carries no decision metadata.
