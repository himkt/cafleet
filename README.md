...
- **Member Lifecycle** -- `cafleet member create/delete/list/capture/send-input/safe-exec` commands wrap tmux pane spawning + agent registration into atomic operations; the `agent_placements` table persists the agent-to-pane mapping in the registry. `member send-input` is a safe `tmux send-keys` wrapper for answering an `AskUserQuestion` prompt (digit 1/2/3 or free text) that mirrors the `capture` authorization boundary. Director-side usage is AskUserQuestion-delegated: the Director asks the user via its own `AskUserQuestion` call and then invokes the resolved command via Bash — see [`skills/cafleet/SKILL.md`](skills/cafleet/SKILL.md) "Answer a member's AskUserQuestion prompt" for the canonical three-beat workflow and pane-shapes table
- **Permission-aware Bash Dispatch** -- `cafleet member safe-exec --bash CMD` is the Director's bash-dispatch primitive. Every invocation re-reads three `settings.json` files (project-local, project shared, user) and matches the inner CMD against the operator's existing `Bash(...)` allow / deny patterns before keystroking anything. Allow → keystroke `! <cmd>` into the member pane (exit 0). Deny → reject with the matched pattern, file, and offending command on stderr (exit 2). Ask (no allow pattern matches) → list the three searched files plus a suggested `Bash(...)` pattern on stderr (exit 3). See [`skills/cafleet/SKILL.md`](skills/cafleet/SKILL.md) "Routing Bash via the Director" for the end-to-end protocol
- **Member Spawn Permission Posture** -- `cafleet member create` always launches the spawned `claude` process with `--permission-mode dontAsk` so the member's Bash tool is enabled and permission prompts auto-resolve silently. Members run cafleet (and any other shell command) directly via the Bash tool. The bash-via-Director protocol (member sends a plain CAFleet message asking the Director to run a command, Director dispatches via `cafleet member safe-exec --bash`) is the fallback when the harness deny-list rejects a Bash invocation (e.g. `git push`, `rm -rf`); members auto-route in that case after first reconsidering whether the command is correct and necessary. See [`skills/cafleet/SKILL.md`](skills/cafleet/SKILL.md) "Routing Bash via the Director" for details
...
| `cafleet --session-id <id> member safe-exec --agent-id <id> --member-id <id> --bash "<cmd>"` | Required | Permission-aware bash dispatch (Director only). Re-reads three `settings.json` files on every call, matches the inner CMD against `Bash(...)` allow / deny patterns, and on allow keystrokes `! <cmd>` into the member pane. Deny exits 2; ask (no allow pattern matches) exits 3 with the searched file list and a suggested pattern. See [docs/spec/cli-options.md](docs/spec/cli-options.md#member-safe-exec) and [`skills/cafleet/SKILL.md`](skills/cafleet/SKILL.md) "Routing Bash via the Director" |

#### `member safe-exec` allow / deny / ask example

`safe-exec` decides every dispatch against the union of `Bash(...)` patterns from `<cwd>/.claude/settings.local.json`, `<cwd>/.claude/settings.json`, and `$CLAUDE_CONFIG_DIR/settings.json` (or `~/.claude/settings.json` if unset) — re-read on every call, deny-wins-on-conflict. With `Bash(git status:*)` in the operator's allow list, `cafleet ... member safe-exec --bash "git status --short"` keystrokes `! git status --short` into the member's pane and exits 0. With `Bash(git push:*)` in the operator's deny list, `cafleet ... member safe-exec --bash "git push origin main"` exits 2 with `Error: command rejected by deny pattern Bash(git push:*) declared in <file>. Offending command: git push origin main` on stderr — nothing is dispatched. With no matching allow pattern for `cafleet ... member safe-exec --bash "rg foo"`, the command exits 3 with the three searched file paths and a suggested `Bash(rg:*)` pattern on stderr — the operator adds the pattern to one of the three files and re-runs. `cafleet --json member safe-exec ...` emits the same five-key JSON payload (`outcome`, `matched_pattern`, `matched_file`, `offending_substring`, `searched_files`) for all three outcomes.
