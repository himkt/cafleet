...
- **Bash Routing via Director** -- `cafleet member create --no-bash` (default) appends `--disallowedTools "Bash"` to the spawned `claude` process so the member's harness rejects Bash calls. When the member needs a shell command, it sends a plain CAFleet message to its Director; the Director responds with `! <command>` keystrokes via `cafleet member send-input`, and Claude Code's `!` shortcut handles execution natively. See [`skills/cafleet/SKILL.md`](skills/cafleet/SKILL.md) "Routing Bash via the Director" for details
