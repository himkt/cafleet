...
- **Bash Routing via Director** -- `cafleet member create --no-bash` (default for claude members) appends `--disallowedTools "Bash"` to the spawned `claude` process so the member's harness rejects Bash calls; the member is expected to route shell commands through its Director via a `bash_request` JSON envelope sent through `cafleet send`. The Director runs the request via `cafleet bash-exec` and replies with a `bash_result` — every operator-consent prompt fires in the Director's pane, never scattered across N member panes. See [`skills/cafleet/SKILL.md`](skills/cafleet/SKILL.md) "Routing Bash via the Director" for the full dispatch flow, payload schemas, and required `Bash(cafleet bash-exec *)` `permissions.allow` setup
...
| `cafleet bash-exec --cmd <text> [--cwd <path>] [--timeout <int>] [--stdin <text>]` | Director-side helper: run a single shell command with deterministic limits (64 KiB stdout/stderr caps, SIGKILL at the requested timeout) and emit a JSON result on stdout. Invoked through the Director's own Bash tool when dispatching a `bash_request` from a member. Does not require `--session-id`. See [`skills/cafleet/SKILL.md`](skills/cafleet/SKILL.md) "Routing Bash via the Director" |
...
| `cafleet --session-id <id> member create --agent-id <id>` | Required | Register a member agent and spawn its tmux pane (Director only). `--coding-agent claude\|codex` selects the backend (default: `claude`). `--no-bash` (default for claude) denies the Bash tool at spawn time so the member routes shell commands through the Director via the `bash_request` envelope; `--allow-bash` opts out (see [`skills/cafleet/SKILL.md`](skills/cafleet/SKILL.md) "Routing Bash via the Director") |
