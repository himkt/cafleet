...
A Director supervises its team on a periodic heartbeat supplied by `cafleet monitor` — a per-fleet `scan → ping → sleep` loop that a coding agent runs as a background task, waking due agents by keystroking `message poll` into their panes. Because the loop is just a backgrounded command, a Director on **any** backend gets the same heartbeat. Run it from the Director's pane as a background task with `cafleet --fleet-id <id> monitor start` (`monitor status` / `monitor config` round out the surface); stop it by stopping that background task, and `fleet delete` makes the loop self-terminate. See <https://himkt.github.io/cafleet/concepts/monitoring/>.
