...
- **Member Lifecycle** -- `cafleet member create/delete/list/capture/send-input/exec/ping` commands wrap tmux pane spawning + agent registration into atomic operations; the `agent_placements` table persists the agent-to-pane mapping in the registry. `member send-input` is a safe `tmux send-keys` wrapper for answering an `AskUserQuestion` prompt (digit 1/2/3 or free text) that mirrors the `capture` authorization boundary. `member exec` is the Director-only shell-dispatch primitive (positional `COMMAND` argument; keystrokes `! <cmd>` + Enter into the member's pane via Claude Code's `!` shortcut). `member ping` is the Director-only manual inbox-poll nudge primitive — pre-approved in `permissions.allow` because the keystroke shape is fixed (`cafleet message poll`); it is the manually-invokable counterpart of the broker's auto-fire. Director-side `send-input` usage is AskUserQuestion-delegated: the Director asks the user via its own `AskUserQuestion` call and then invokes the resolved command via Bash — see [`skills/cafleet/SKILL.md`](skills/cafleet/SKILL.md) "Answer a member's AskUserQuestion prompt" for the canonical three-beat workflow and pane-shapes table
- **Member Spawn Permission Posture** -- `cafleet member create` always launches the spawned `claude` process with `--permission-mode dontAsk` so the member's Bash tool is enabled and permission prompts auto-resolve silently. Members run cafleet (and any other shell command) directly via the Bash tool. The bash-via-Director protocol (member sends a plain CAFleet message asking the Director to run a command, Director dispatches via `cafleet member exec`) is the fallback when the harness deny-list rejects a Bash invocation (e.g. `git push`, `rm -rf`); members auto-route in that case after first reconsidering whether the command is correct and necessary. See [`skills/cafleet/SKILL.md`](skills/cafleet/SKILL.md) "Routing Bash via the Director" for details
...
| `cafleet --session-id <id> member send-input --agent-id <id>` | Required | Forward a restricted keystroke (`--choice {1,2,3}` or `--freetext "<text>"`) to a member's AskUserQuestion prompt (Director only); see [docs/spec/cli-options.md](docs/spec/cli-options.md#member-send-input). The workflow is AskUserQuestion-delegated — see [`skills/cafleet/SKILL.md`](skills/cafleet/SKILL.md) "Answer a member's AskUserQuestion prompt" |
| `cafleet --session-id <id> member exec --agent-id <id> --member-id <id> "<cmd>"` | Required | Dispatch a shell command into a member's pane via Claude Code's `!` shortcut (Director only); the bash-via-Director fallback when the member's harness deny-list rejects a Bash invocation. See [`skills/cafleet/SKILL.md`](skills/cafleet/SKILL.md) "Routing Bash via the Director" |
| `cafleet --session-id <id> member ping --agent-id <id> --member-id <id>` | Required | Inject a `cafleet message poll` keystroke into a member's pane (Director only); manually-invokable counterpart of the broker's auto-fire. Pre-approved in `permissions.allow` because the keystroke shape is fixed. See [`skills/cafleet/SKILL.md`](skills/cafleet/SKILL.md) "Member Ping" |
