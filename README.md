...
- **Bash Routing via Director** -- `cafleet member create --no-bash` (default for claude members) appends `--disallowedTools "Bash"` to the spawned `claude` process so the member's harness rejects Bash calls; the member is expected to route shell commands through its Director via a `bash_request` JSON envelope sent through `cafleet message send`. The Director runs the request via `cafleet member exec` and replies with a `bash_result` — every operator-consent prompt fires in the Director's pane, never scattered across N member panes. See [`skills/cafleet/SKILL.md`](skills/cafleet/SKILL.md) "Routing Bash via the Director" for the full dispatch flow, payload schemas, and required `Bash(cafleet member exec *)` `permissions.allow` setup
...
| `cafleet --session-id <id> member create --agent-id <id>` | Required | Register a member agent and spawn its claude tmux pane (Director only). `--no-bash` (default) denies the Bash tool at spawn time so the member routes shell commands through the Director via the `bash_request` envelope; `--allow-bash` opts out (see [`skills/cafleet/SKILL.md`](skills/cafleet/SKILL.md) "Routing Bash via the Director") |
...
| `cafleet member exec --cmd <text> [--cwd <path>] [--timeout <int>] [--stdin <text>]` | Not required | Director-side helper: run a single shell command with deterministic limits (64 KiB stdout/stderr caps, SIGKILL at the requested timeout) and emit a JSON result on stdout. Invoked through the Director's own Bash tool when dispatching a `bash_request` from a member |
