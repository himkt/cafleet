...
| 2026-04-29 | **Round 11 (Member-side `!` prefix wiring per design 0000035)**: Design 0000035 was filed mid-PR-#37 review to address the implicit-`!`-shortcut gap discovered during real-world smoke tests of the bash-via-Director feature: members spawned with `--disallowedTools "Bash"` could not reliably invoke `cafleet message poll/send/ack` themselves because the spawn-prompt template never told them to use the `!` CLI shortcut, and the broker's tmux push-trigger keystroke arrived bare (without `! `). Three alternative spawn-argv strategies were smoke-tested (`--allowedTools "Bash(cafleet *)"` leaks all bash; a `--settings` deny+allow pair removes Bash entirely; the status quo forces an implicit `!` workaround) — none yielded a clean strict allow-list. Option A was chosen for v1: keep the deny posture and document/wire the `!` prefix everywhere. R11 lands the 0000035 implementation **in this PR (#37) alongside the rest of the 0000034 work**, not in a separate follow-up. **Step 1**: `cafleet/src/cafleet/coding_agent.py` `CLAUDE.default_prompt_template` now tells the spawned member to prefix every cafleet call (poll/send/ack) with `! ` and explicitly forbids using the Bash tool for cafleet; both pre-existing canaries (`"If your Bash tool is denied"`, `"! command"`) preserved, new canary `"! cafleet"` added. **Step 2**: `cafleet/src/cafleet/tmux.py` `send_poll_trigger` now keystrokes `f"! cafleet --session-id {session_id} message poll --agent-id {agent_id}"` (with leading `! `) so the broker-pushed notification arrives in a form the harness routes through Claude Code's `!` CLI shortcut without going through the Bash tool. Docstring updated. **Step 3**: doc sweep — `skills/cafleet/SKILL.md` § Routing Bash via the Director gained a new subsection "Member side: prefix every cafleet call with `!`" covering the `--no-bash` deny posture, the `!` shortcut as a Claude Code primitive independent of the Bash deny, and the four cafleet sub-commands the member needs (poll/send/ack/cancel); `ARCHITECTURE.md` § Bash Routing via Director gained a paragraph mirroring the same content; this changelog entry. **Test count delta**: 517 → 518 passing (1 new canary `TestPromptTemplates::test_claude_template_documents_bang_prefix_for_cafleet`; Step 2 modified an existing test, no count change). All quality gates pass: 518 tests, ruff check clean, ruff format clean, ty check clean. See [`design-docs/0000035-member-bash-whitelist/design-doc.md`](../0000035-member-bash-whitelist/design-doc.md) for the full design — Option A was chosen; Options B (PreToolUse hook) and C (proxy binary `cafleet-member`) are deferred to Future Work. Round 11 is post-Complete maintenance like Rounds 8 and 9 — Status remains Complete and Progress remains 58/63. |
