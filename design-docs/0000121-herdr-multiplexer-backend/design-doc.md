...
| `split_window(reference, env, command)` — `split-window -t <window> -d … <argv>` + `select-layout main-vertical` (reflows all panes: Director main-left, members stacked right) | Layout-aware (no herdr reflow command exists): if the Director's pane (`reference.pane_id`) has no right neighbor → `herdr pane split <reference.pane_id> --direction right --no-focus` (first member starts the right column); else walk `herdr pane neighbor --direction right/down` to the bottom-of-column pane and `herdr pane split <bottom> --direction down --no-focus`. Then equalize the right column with `herdr pane resize` (best-effort). Finally `herdr pane run <new_id> "<shlex.join(command)>"`. `[--env K=V …]` forwarded on the split |
...
- **Layout parity (Director main-left + right-column stack, equal heights).** tmux calls `select-layout main-vertical` after each split, which reflows every pane into the canonical arrangement (Director in the large left pane, members stacked in the right column). herdr has **no layout-reflow command**, so `HerdrMultiplexer.split_window` reproduces the same visual arrangement explicitly and statelessly, discovering the current layout through `herdr pane neighbor --direction …` (not pane geometry fields): (1) if the Director's pane has **no** right neighbor, this is the first member — `herdr pane split <reference.pane_id> --direction right --no-focus` starts the right column; (2) otherwise, walk `herdr pane neighbor --direction right` from the Director to the top of the column, then `--direction down` to its bottom pane, and `herdr pane split <bottom> --direction down --no-focus` appends below. After the split, the right-column panes are equalized to the same height with `herdr pane resize` (best-effort). The exact `herdr pane split --ratio` / `herdr pane resize --amount` semantics and whether a split auto-balances the column are **not pinned by herdr's public docs** and are validated against a real herdr binary (same posture as the `pane read` result key); the structural navigation via `pane neighbor` is deterministic. tmux is untouched (still `select-layout main-vertical`).
...
COMMENT(programmer): Herdr layout parity implemented in `split_window` (`_neighbor`/`_split_pane`/`_equalize_column`), verified via lint+typecheck. Choices pending operator validation (herdr.dev CLI reference consulted): (a) **target selection** — `pane neighbor` and `pane resize` use `--pane <id>` (the reference documents `--pane ID|--current`, not positional); `pane split` keeps the positional `<pane_id>` (docs example confirms). (b) **no-neighbor signal** — CONFIRMED live (operator): `pane neighbor` with no neighbor exits 0 with an absent/null `pane` key (NOT an error); `_neighbor` returns None when `result.get("pane")` is not a dict, and lets a genuine `HerdrError` propagate. (c) **resize `--amount`** — the reference says it "adjusts size by a FLOAT" (delta-vs-target unclear); `_equalize_column` passes `--direction down --amount <1/N>` per pane, best-effort/swallowed — if `--amount` is a delta rather than a target fraction, heights won't be exact. (d) **`pane split --ratio`** — exists but omitted (herdr default split ratio); could tune the Director-vs-column proportion. Step 7's `test_herdr.py` pins the neighbor/split/resize argv sequence incl. the first-member (`--direction right`) vs subsequent (`--direction down`) branch.
