...
| `split_window(reference, env, command)` — `split-window -t <window> -d … <argv>` + `select-layout main-vertical` (reflows all panes: Director main-left, members stacked right) | Layout-aware (no single herdr reflow command exists): read `herdr pane list`; the right column = panes in the Director's tab (`tab_id == reference.window_id`) minus the Director's own pane. If empty → `herdr pane split <reference.pane_id> --direction right --no-focus` (first member starts the right column); else `herdr pane split <max(column)> --direction down --no-focus`, then `_equalize_focused_tab_column()` rebalances the column to equal heights (see the *Layout parity* note). Finally `herdr pane run <new_id> "<shlex.join(command)>"`. `[--env K=V …]` forwarded on the split |
...
- **Layout parity (Director main-left + right-column stack).** tmux calls `select-layout main-vertical` after each split, which reflows every pane into the canonical arrangement (Director in the large left pane, members stacked in the right column). herdr has **no single layout-reflow command**, so `HerdrMultiplexer.split_window` reproduces the same arrangement in two deterministic steps, discovering the current layout through **`herdr pane list`** (`result.panes[]`, each with `pane_id` / `tab_id` — the confirmed live JSON shape). It computes the current right column as the panes in the Director's own tab (`tab_id == reference.window_id`) excluding the Director's own pane: (1) if that set is **empty**, this is the first member — `herdr pane split <reference.pane_id> --direction right --no-focus` starts the right column; (2) otherwise split `max(column)` `--direction down --no-focus` to append another row below, **then equalize the column to equal heights** via `_equalize_focused_tab_column()`. `pane list` is used rather than `pane neighbor` because the neighbor command did not reliably report an existing neighbor in live testing; the pane-list membership test is deterministic. **Height equalization** is arithmetic, not inference: read the focused tab id (`herdr pane current` → `result.pane.tab_id`) and the tab geometry (`herdr pane layout` → `result.layout.{panes[].rect, splits[].{direction,rect,ratio}}`); the right column (panes whose `rect.x` is not the minimum x) forms a right-leaning chain of `down` splits where split *k* (top→bottom) separates member *k* from the members below. Equal heights ⇔ split *k* has ratio `1/(N-k)`; since `herdr pane resize --amount` is a signed delta on a split's ratio (confirmed 1:1 against the live binary), each split is driven to target by one resize with `delta = 1/(N-k) - ratio` (`delta > 0` → resize member *k* `--direction down`; else member *k+1* `--direction up` by `|delta|`; deltas `< 1e-3` are skipped). Equalization is **best-effort** — any `HerdrError` is swallowed so a resize never fails a spawn over cosmetics. tmux is untouched (still `select-layout main-vertical`).
...
- [x] Herdr layout parity: `HerdrMultiplexer.split_window` reproduces tmux's main-vertical arrangement (Director main-left, members stacked in the right column) per the §1 *Layout parity* note — a `pane list` membership test picks the anchor (first member → `--direction right` off the Director; subsequent → `--direction down` off `max(column)`), then `_equalize_focused_tab_column()` rebalances the column to equal heights by driving each `down` split to ratio `1/(N-k)` via `herdr pane resize` (arithmetic delta, best-effort). Confirmed working in a live herdr session. <!-- completed: 2026-07-06T12:56 -->
