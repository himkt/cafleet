...
- **Layout parity (Director main-left + right-column stack, equal heights).** tmux calls `select-layout main-vertical` after each split, which reflows every pane into the canonical arrangement (Director in the large left pane, members stacked in the right column). herdr has **no layout-reflow command**, so `HerdrMultiplexer.split_window` reproduces the same visual arrangement explicitly and statelessly, discovering the current layout through **`herdr pane list`** (`result.panes[]`, each with `pane_id` / `tab_id` — the confirmed live JSON shape). It computes the current right column as the panes in the Director's own tab (`tab_id == reference.window_id`) excluding the Director's own pane: (1) if that set is **empty**, this is the first member — `herdr pane split <reference.pane_id> --direction right --no-focus` starts the right column; (2) otherwise split an existing column pane `--direction down --no-focus` to append another row, then equalize the column heights with `herdr pane resize` (best-effort). `pane list` is used rather than `pane neighbor` because the neighbor command did not reliably report an existing neighbor in live testing; the pane-list membership test is deterministic. The `herdr pane resize --amount` semantics (delta vs target fraction) and the vertical ordering of stacked members are **not pinned by herdr's public docs** and are best-effort, validated against a real herdr binary (same posture as the `pane read` result key). tmux is untouched (still `select-layout main-vertical`).
