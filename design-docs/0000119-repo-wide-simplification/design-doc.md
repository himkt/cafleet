...
COMMENT(director): Programmer assignment (batch 4, final source batch) — batches 1-3 committed (admin-ui, docs, skills). Batch 4 = ALL remaining source-code areas plus their paired SPEC.md / docs/api/broker.md edits. You own SOURCE only; the Tester owns every test file and has already rewired the test call sites for the mixed findings (broker-db-1/2/3, cli-1, output-webui-6) to the surviving APIs — so when you delete a symbol, do NOT touch its tests. Areas:
- Step 3 cli: cli-1 (source _helpers.py + SPEC.md:908-926; do NOT touch tests/cli/test_client_command.py), cli-2, cli-3, cli-4, cli-5, cli-6, cli-7, cli-8.
- Step 2 broker-db source: broker-db-5, broker-db-6, broker-db-7 (+ SPEC.md:761-762), broker-db-4 (+ docs/api/broker.md), and the SOURCE deletes of broker-db-1 (delete is_monitoring_member from _shared.py + SPEC.md three mentions), broker-db-2 (delete broker.list_agents + __init__ entries + SPEC.md §6.2 + docs/api/broker.md), broker-db-3 (delete record_ping + __init__ lines + SPEC.md:737-738 + docs/api/broker.md).
- Step 5 output-webui source: output-webui-1 (extract broker.monitor_runtime_payload, rewire cli/monitor.py + webui/api.py + docs/api/broker.md + SPEC ~:770), output-webui-4 (delete _agent_id_for_column, inline at 2 callers + SPEC.md:1472-1473), and output-webui-6 SOURCE (drop limit param from truncate_task_text in output/render.py + SPEC.md:1463/:1482; do NOT touch tests/output/test_output.py).
- Step 7 runtime source: runtime-1, runtime-2, runtime-4 (+ SPEC.md §6.5 ~:1738).
SHARED-FILE COORDINATION (per the design doc's Specification > Delivery and ordering): SPEC.md is edited by NINE findings and docs/api/broker.md by FOUR — the cited line numbers are authoring-time anchors that DRIFT as edits land; re-locate each edit by its section/symbol at apply time, and apply each shared file's edits in one coordinated pass so the collective diff is internally consistent. Consult scan/findings.md for full evidence. When done, run mise //cafleet:lint and mise //cafleet:typecheck yourself and report their status; do NOT run the full pytest suite (the Director runs it once your source + the Tester's tests are both in). Report complete per step (Step 3: cli / Step 2: broker-db / Step 5: output-webui / Step 7: runtime), or one complete (doc) — batch4 with the four step names, whichever fits.
