...
**Status**: Approved
**Progress**: 14/62 tasks complete
**Last Updated**: 2026-07-04

COMMENT(director): Tester assignment (batch 2) — batch 1 done and committed. You (Tester) own ALL test-file changes exclusively; there is no cross-member file conflict, so sequence your own test-file work freely. Do NOT edit non-test files and do NOT edit this design doc. Batch 2 is your ENTIRE remaining test queue: (a) test-internal refactors tests-2, tests-3, tests-4, tests-5, tests-7, tests-8; (b) the test-side halves of the mixed findings broker-db-1 (prune is_monitoring_member tests: reduce _PREDICATES in test_kind_predicate_safety.py to is_administrator, drop test_is_monitoring_member__kind_detection), broker-db-2 (migrate the 8 test call sites off broker.list_agents to broker.list_roster / list_fleet_agents — both targets already exist), broker-db-3 (change the 5 test call sites to broker.record_pings([id], when) — already exists), cli-1 (update tests/cli/test_client_command.py test-only commands to the single remaining truncate+formatter mode), output-webui-6 (rewire the 6 tests/output/test_output.py call sites to set the limit via the settings mechanism as test_truncation_settings.py does). RESPECT the ordering constraints in the design doc's Specification > Delivery and ordering section, which you have read: apply tests-5's conftest additions BEFORE tests-7's fixture adoption; confirm the autouse _cli_registry interaction holds after tests-2; coordinate edits to tests/broker/test_monitor.py so broker-db-1/3 prunes and the tests-3/4/8 shared-helper migrations do not conflict. The test-side migrations depend on NOTHING from the Programmer (all target APIs already exist), so proceed independently. Consult scan/findings.md for full evidence. Do NOT run the full test suite yourself — the Director runs the gates after both members land. Report complete (doc) — batch2 (or per sub-group: complete (doc) — tests-refactors / complete (doc) — broker-db-tests / complete (doc) — mixed-rewires) so the Director can gate incrementally.

COMMENT(director): Programmer assignment (batch 2) — batch 1 (admin-ui) done and committed. You (Programmer) own ALL source/doc/config/skill changes exclusively; the Tester owns all test files (tests/**, **/test_*.py, **/*_test.py) — do NOT edit any test file. Do NOT edit this design doc (the Director owns checkbox/Progress updates). For every finding that pairs a source removal with a SPEC.md / docs/ / skills edit, make the paired doc edit in the SAME work (per .claude/rules/removal.md). Batch 2 = the isolated doc + config areas: Step 6 project-config (project-config-2, project-config-3, project-config-4) and Step 4 docs (docs-1, docs-3, docs-4, docs-5, docs-6, docs-7). Note docs-3 also edits README.md §4/§5 and .claude/skills/update-readme/SKILL.md. Consult scan/findings.md for full evidence. Do NOT run the Python test suite. Report complete per step: complete (paragraph-Implementation > Step 6: project-config) and complete (paragraph-Implementation > Step 4: docs). Later batches (skills-cafleet + skills-workflows, then cli/broker-db/output-webui/runtime source incl. shared SPEC.md + docs/api/broker.md edits) follow.
