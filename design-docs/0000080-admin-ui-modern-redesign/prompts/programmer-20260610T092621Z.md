...
TEAM NOTE: There is no Tester on this team — the design doc explicitly specifies lint + typecheck + build + manual visual verification instead of new test infra. A Verifier member handles E2E verification after implementation. Quality gates you must run yourself per step: mise //admin:lint and mise //admin:build (run from the repo root; never cd into admin/ except where the design doc requires plain `bun install` inside admin/ to regenerate the lockfile).
