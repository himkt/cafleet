...
COMMENT(programmer): Step 2 implementation is done (config v3, models renamed, fresh `0001` regenerated and hand-reviewed), but the step cannot go green as specified — two issues need arbitration. (1) **Test defect**: `tests/db/test_alembic_smoke.py` at HEAD is byte-identical to `main` — it still asserts the old agent-named schema (`agents` in expected tables, `agent_placements` with a nullable `director_agent_id` + `idx_placements_director`, `monitor_config.agent_id`, `tasks.to_agent_id`, agents-FK checks), while § "Database schema (fresh DB, hard break)" specifies the fresh `0001` creates the members schema (`members`, `member_placements` without a director column, `member_id`, `to_member_id`, `idx_members_fleet_status`). The Tester needs to rename those assertions; I must not edit test files. (2) **Sequencing**: even the chain-guard test cannot run at the end of Step 2 — `tests/conftest.py` imports `cafleet.broker`, whose `_shared.py` still does `from cafleet.db.models import Agent`, so collection fails with `ImportError: cannot import name 'Agent'` until Step 3's broker rename lands. Either accept a red suite until Step 3 completes or fold the Step-2 test run into Step 3's gate.

COMMENT(director): (1) Tester renames every smoke-test schema assertion to the members schema per § Database schema — the fresh 0001 is the contract, the old assertions are the defect. (2) Accepted: the suite stays red/uncollectable until Step 3's broker rename lands; the per-step green gate is waived for Steps 2–3 and full-suite green remains the Step 6 gate. Tester removes both markers as part of the fix.

- [ ] Bump the default DB filename to `cafleet_v3.db` in `config.py` (`_default_database_url` factory and both docstrings) <!-- completed: -->
- [ ] Update `db/models.py`: `Member` / `MemberPlacement`, renamed columns and indexes, `director_agent_id` column + `idx_placements_director` deleted <!-- completed: -->
- [ ] Delete `db/alembic/versions/0001_initial_schema.py` and regenerate the fresh initial `0001` via `mise //cafleet:makemigration "initial schema"` from the renamed models; review per `database-migrations.md` <!-- completed: -->
