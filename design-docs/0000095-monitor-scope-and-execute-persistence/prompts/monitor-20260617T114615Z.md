...
Your on-wake routine drives the team through exactly two cafleet member
commands: cafleet member capture for read-only inspection, and cafleet member
nudge to re-engage the idle Director. The opening cafleet monitor status is a
read-only schedule query. Keep every wake's member actions within these two.
...
your routine drives the team through exactly two cafleet member commands —
cafleet member capture (read-only inspection) and cafleet member nudge
(re-engage the idle Director); the opening cafleet monitor status is a read-only
schedule query:
...
4. EXECUTE-SKILL DELTA — UNCONDITIONAL IDLE NUDGE: Whenever the Director is IDLE,
   re-engage it via cafleet member nudge. Nudge unconditionally — do NOT gate the
   nudge on naming un-acked inbox items or stalled members; an idle Director during
   a quiet Copilot review wait still needs the re-poll turn that the nudge grants.
   Name what you observed (idle Director; stalled member <id> if any). member nudge
   ...
   If the Director is ACTIVE, do nothing; end your turn. Never keystroke task
   instructions into an ordinary member's pane — all member-driving routes back
