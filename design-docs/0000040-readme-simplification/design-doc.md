...
- [x] `README.md` is restructured around three top-level sections in this order: **Install (plugin)** → **Try it (example prompt)** → **CLI cheatsheet** (one consolidated short table). Other sections (architecture link, development, license) are present but compact.
- [x] Total `README.md` length is ≤ 180 lines (final: 81 lines per `wc -l`).
- [x] The Features list (currently 20 changelog-style bullets, README L11–L28) is replaced by **4–6 high-level bullets** OR removed entirely if the example prompt carries the value. Decision recorded in commit `149386a`: removed entirely (Q4 path B).
- [x] The two large CLI tables (Server Administration, Agent Commands — README L207–L238) are replaced by **one consolidated table with one row per command group** (`session`, `agent`, `message`, `member`, `db`, `server`, `doctor`) plus a single link to `docs/spec/cli-options.md`. The link wording in the rewritten README scopes the reference accurately: `cli-options.md` has dedicated per-command sections for `session`/`member`/`doctor`/`server`; `agent`/`message`/`db init` are covered via the option-source table and `cafleet <cmd> --help` (per the §6 audit caveat).
- [x] Every paragraph documenting *internal mechanics* — Administrator protection, soft-delete cascade, root-Director protection, push-notification mechanics, env-var precedence, Alembic state matrix, `agent_placements` semantics, `--permission-mode dontAsk`, the Bash-via-Director fallback, Claude Code `permissions.allow` rationale — is removed from `README.md`. None of that detail is paraphrased back; readers who need it follow a single link to `ARCHITECTURE.md`.
- [x] The plugin install section uses the canonical Claude Code plugin install command shape verified against `.claude-plugin/plugin.json` (manifest `name = cafleet`, `repository = https://github.com/himkt/cafleet`, declares 5 skills under `./skills/*`). The command shape is `/plugin marketplace add himkt/cafleet` followed by `/plugin install cafleet@himkt-cafleet` — verified via `WebFetch` of `https://docs.claude.com/en/docs/claude-code/plugins` (redirected to `code.claude.com/docs/en/discover-plugins`); the marketplace identifier auto-derives from `owner-repo` with **hyphen**, not slash.
- [x] The example-prompt section shows **one** end-to-end prompt that a user types into Claude Code after the plugin is installed, demonstrating one of the plugin's high-value skills (the recommended hero is `/cafleet:design-doc-create <slug>` because it exercises every primitive — session bootstrap, member spawn, message broker, monitoring loop, teardown — through a single user prompt). Showing the full prompt the user types is sufficient — no transcript of the agent's response.
- [x] Light audit pass over `ARCHITECTURE.md` and `docs/spec/` ensures every fact `README.md` is dropping is already documented (or moved into) one of those two surfaces. No content is lost in the cut. Audit findings + any patches are recorded in the implementation Step 1 below.
- [x] Proactive sweep of every `skills/*/SKILL.md` (5 skills: `cafleet`, `cafleet-monitoring`, `design-doc`, `design-doc-create`, `design-doc-execute`) for links or anchors pointing into README sections that are deleted. Any matches are repointed to `ARCHITECTURE.md` or `docs/spec/cli-options.md`. Sweep is recorded as a checked task in Step 4 with the file:line list of matches (or "no matches found").
- [x] All four mise tasks pass after the change: `mise //cafleet:lint`, `mise //cafleet:format`, `mise //cafleet:typecheck`, `mise //cafleet:test`. (Pre-existing format drift in `tests/test_cli_member_ping.py` was normalized in a separate `chore:` commit so format produces no diff.)
...
- **Row 6 (Two CLI tables — Server Administration L207–L213, Agent Commands L220–L238)** — covered, but with a coverage caveat. `docs/spec/cli-options.md` has dedicated per-command sections for `session create/list/show/delete` (L74–L188), `member create/delete/list/capture/send-input/exec/ping` (L281–L587), `doctor` (L190–L232), `server` (L234–L280), plus an Error Messages section (L589). It does NOT yet have dedicated per-command sections for `db init`, `agent register/deregister/list/show`, or `message send/broadcast/poll/ack/cancel/show`; those are listed in the Agent ID summary table (L51–L73) and otherwise covered by `cafleet <cmd> --help`. The new README's `## CLI cheatsheet` link wording must reflect this: it links to the file as a CLI reference, not as an exhaustive per-subcommand reference. No patch to `cli-options.md` is added here (out of scope per Q7); the README link wording is what carries the accuracy.
...
| 2026-04-30 | Reviewer round 1: fix initial Progress field from 0/18 to 0/23, split Step 1 (draft) and Step 2 (apply) to remove duplication, reorder steps to honor design-doc-numbering rule (ARCH/docs apply before README rewrite), pin verbatim README content for §3 example block, name the canonical Claude Code plugins documentation URL for §2 plugin install verification. |
| 2026-04-30 | Approved by user. Status set to Approved; ready for /design-doc-execute. |
| 2026-04-30 | Implementation complete (PR #43). All 23 tasks checked. README.md cut from 333 to 81 lines (Features removed entirely, single 7-row CLI cheatsheet, plugin install verified canonical hyphen form `cafleet@himkt-cafleet`). 0 hard gaps in ARCH/docs audit; 0 matches in SKILL.md sweep. Copilot review round 1 (commit 876e112): 3 comments addressed — design-doc fence depth + slash-vs-hyphen text in §2; README mise lint comment expanded to include `ruff format --check`. Round 2 (commit 4074e10): 6 comments addressed — line count 80→81; Row 6 audit caveat (cli-options.md only has dedicated sections for session/member/doctor/server); changelog wording; `cafleet db init` "one-time" → "idempotent; rerun after upgrades" in both Install and cheatsheet rows; CLI reference link wording softened. Round 3 (commit 99e49bc): 3 new actionable comments addressed — SKILL state-table scope clarified as PR-review-specific; SKILL silence-escalation gate (7f) added so the loop never auto-exits on Copilot silence but does prompt the user after ~30 silent ticks; this changelog entry rewritten to remove the inaccurate "Round 2 quiescent" claim. Status set to Complete. |
