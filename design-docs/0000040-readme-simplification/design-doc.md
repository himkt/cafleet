...
- [x] The two large CLI tables (Server Administration, Agent Commands — README L207–L238) are replaced by **one consolidated table with one row per command group** (`session`, `agent`, `message`, `member`, `db`, `server`, `doctor`) plus a single link to `docs/spec/cli-options.md` for the full per-subcommand surface.
