...
- [x] ~~Add a helper `_match_known_task_pattern(path: Path, repo_root: Path)`~~ — superseded. The resolver was simplified to be skill-agnostic per user directive; absolute paths under the repo root are treated as the task folder verbatim, no bucket walk. <!-- completed: 2026-05-16T01:55 (helper later removed in architectural simplification) -->
