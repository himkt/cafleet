...
| The `cafleet base-dir resolve --path` CLI option | **Removed, not deprecated.** Per `~/.claude/rules/removal.md`, every mention of `--path` in source code, skills, README, ARCHITECTURE, and docs is deleted in the same change set. No alias, no deprecation warning, no historical reference. |
