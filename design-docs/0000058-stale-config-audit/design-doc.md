...
| `.claude-plugin/plugin.json` | `.claude-plugin/plugin.json` | v0.8.1 (bumped by this design — see Step 5h) with 12 `./skills/<name>` entries matching the 12 skill directories under `skills/`. LIVE. | PATCH bump from 0.8.0 → 0.8.1 per design 0000054 §145 (existing skill-body edits require a patch bump). |
