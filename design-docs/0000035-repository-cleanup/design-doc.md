...
| `docs/spec/webui-api.md` | 13 | Sentence `"No server-side session. No Auth0. The SPA manages the active session_id client-side via hash-based routing."` advertises Auth0 removal. | UPDATE | Drop the `"No Auth0. "` clause. Resulting line: `"No server-side session. The SPA manages the active session_id client-side via hash-based routing."` |
