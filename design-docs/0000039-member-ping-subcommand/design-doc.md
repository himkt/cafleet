...
- [x] `cafleet member ping --agent-id <director-id> --member-id <member-id>` exists as a new Director-only subcommand. The Click signature carries exactly two required options (`--agent-id`, `--member-id`) and **no positional argument** — the action is wholly determined by the subcommand name.
...
- [x] The subcommand reuses `_load_authorized_member` for cross-Director boundary enforcement and reuses the existing missing-agent / missing-placement / cross-Director / pending-placement error wording verbatim — same authorization contract as `member capture` / `member send-input` / `member exec`.
- [x] Exit codes: `0` on dispatch success, `1` on tmux failure (`send_poll_trigger` returning `False` is converted to `click.ClickException` with wording `Error: send failed: <details>` so an operator running the command manually — or a monitoring loop wrapping it — sees the failure rather than a silent best-effort skip), `1` on every authorization-boundary rejection (cross-Director, missing placement, pending placement, agent not found, tmux unavailable). There are no exit-2 input-validation failures because the subcommand has no positional / freeform input — Click's built-in "Missing option" error covers omitted `--agent-id` / `--member-id` (exit 2).
- [x] JSON output (`cafleet --json ... member ping ...`) emits exactly two keys: `member_agent_id`, `pane_id`. No `action` field, no `polled` field — the subcommand name IS the action; failures surface via exit 1, not via a `polled: false` field.
- [x] Text output is one line: `Pinged member <name> (<pane_id>) — poll keystroke dispatched.`
- [x] `.claude/settings.json` allows `cafleet member ping` invocations without per-call operator confirmation. Specifically, the existing broad `Bash(cafleet *)` entry in `permissions.allow` already covers `member ping`; no new entry is added (the more specific entry would be redundant). The exec surface stays under `permissions.ask` via the existing `Bash(cafleet * member exec *)` entry (added by design 0000038), which is more specific than `Bash(cafleet *)` and therefore takes precedence — that asymmetric specificity is the entire point of the carve-out: `member exec` matches the `ask` rule and prompts; `member ping` falls through to the broad `allow` and fires.
- [x] `broker._try_notify_recipient` is **not** modified. The new subcommand is strictly additive: an additional manual entry-point that reuses the same `tmux.send_poll_trigger` helper but does not refactor or replace the auto-fire path. The auto-fire stays best-effort (returns `False` silently on tmux failure); the manual `member ping` path converts `False` to exit 1.
- [x] Documentation is updated FIRST per `.claude/rules/design-doc-numbering.md`. Targets: `ARCHITECTURE.md`, `docs/spec/cli-options.md`, `README.md`, `skills/cafleet/SKILL.md`, `skills/cafleet/roles/director.md`, `skills/cafleet/roles/member.md`, `skills/cafleet-monitoring/SKILL.md`, `.claude/rules/bash-tool.md`, `.claude/settings.json`, and the `cafleet/src/cafleet/tmux.py::send_poll_trigger` docstring.
- [x] Every place in the documentation surface that currently advises `cafleet member exec "cafleet ... message poll ..."` (or implies using `member exec` to trigger an inbox-poll) is re-pointed to `cafleet member ping`. The `member exec` surface stays scoped to *arbitrary shell dispatch* (its original 0000038 charter); the inbox-poll usage moves to `member ping`. See §6 Doc-surface re-pointing for the explicit list.
- [x] `cafleet/tests/test_cli_member_ping.py` is created and covers: happy-path dispatch (asserts `tmux.send_poll_trigger` called once with the correct kwargs), text and JSON outputs (two keys), tmux-failure-to-exit-1, missing-agent / missing-placement / cross-Director / pending-placement rejection wording (mirrors `test_cli_member_exec.py`), tmux-unavailable.
...
| **0000038** `exec-subcommand` | Complete | Carved `cafleet member exec` out of `cafleet member send-input --bash` so the strict `permissions.ask` confirmation could be scoped to *arbitrary shell-dispatch* without polluting the AskUserQuestion-answer path. Established the `cafleet member <verb>` carve-out pattern that this design replicates. The `Bash(cafleet * member exec *)` `permissions.ask` rule stays in place. This design relies on rule specificity: `member exec` matches that ask rule (most specific), while `member ping` falls through to the broader `Bash(cafleet *)` rule in `permissions.allow` and fires without confirmation. No new entry is added. |
...
**No source edit is required.** The existing `permissions.allow` rule `Bash(cafleet *)` already matches `cafleet member ping` invocations, and the existing `permissions.ask` rule `Bash(cafleet * member exec *)` (added by design 0000038) is more specific than the broad allow rule and therefore takes precedence for `member exec`. That precedence asymmetry is exactly what the carve-out needs:

- `cafleet member exec ...` → matches `Bash(cafleet * member exec *)` in `ask` → operator confirmation per call.
- `cafleet member ping ...` → falls through to `Bash(cafleet *)` in `allow` → fires without confirmation.

Adding a more specific `Bash(cafleet --session-id * member ping *)` entry to `permissions.allow` would be redundant given the broad `Bash(cafleet *)` entry. The Claude Code permission matcher resolves precedence by specificity (most-specific match wins across `allow`/`ask`/`deny`), and there is no `ask` rule that would otherwise capture `member ping` — so the broad allow is sufficient.

If the project ever tightens `permissions.allow` to enumerate cafleet subcommands explicitly (removing the blanket `Bash(cafleet *)`), the explicit `Bash(cafleet --session-id * member ping *)` entry should be added at that time. That is a separate concern out of scope for this design.
...
- [x] Update `.claude/settings.json`: **no edit needed.** The existing broad `Bash(cafleet *)` entry in `permissions.allow` already matches `cafleet member ping`, and the existing `Bash(cafleet * member exec *)` entry in `permissions.ask` (from design 0000038) is more specific and therefore takes precedence for `member exec`. Adding a `Bash(cafleet --session-id * member ping *)` entry would be redundant. The existing `permissions.ask` entry for `member exec` stays untouched. <!-- completed: 2026-04-30T16:00 (revised after Copilot review on PR #42 — removed the redundant explicit ping entry and aligned wording with the actual `Bash(cafleet * member exec *)` ask rule) -->
...
- [x] Add the new `member_ping` subcommand. Place it directly after `member_exec` to keep the Director-only dispatch primitives clustered. Implementation outline:
  1. `_require_session_id(ctx)`.
  ...
  8. Text output: `f"Pinged member {target['name']} ({pane_id}) — poll keystroke dispatched."`. <!-- completed: 2026-04-30T16:30 -->
- [x] Run `mise //cafleet:lint` and `mise //cafleet:format` to confirm the new code is clean. <!-- completed: 2026-04-30T16:30 -->
...
- [x] `mise //cafleet:test` passes (existing tests still pass; new `test_cli_member_ping.py` passes; `broker._try_notify_recipient` tests still pass unchanged). <!-- completed: 2026-04-30T16:35 -->
- [x] `mise //cafleet:lint` passes. <!-- completed: 2026-04-30T16:35 -->
- [x] `mise //cafleet:format` passes. <!-- completed: 2026-04-30T16:35 -->
- [x] `mise //cafleet:typecheck` passes. <!-- completed: 2026-04-30T16:35 -->
...
| 2026-04-30 | Copilot review on PR #42 (round 2, late-arriving). Applied 3 fixes. (1) `skills/cafleet/roles/director.md` line 86: extended the `member exec` vs `member ping` partition paragraph to explain the rule-specificity precedence (more-specific `Bash(cafleet * member exec *)` ask rule takes precedence over the broad `Bash(cafleet *)` allow), removing the implication that a hardcoded `cafleet --session-id * member exec *` glob is what enforces the carve-out. (2) `skills/cafleet/roles/member.md` line 84: clarified the broker auto-fire wording — the keystroke is injected into the **recipient's** pane after every `cafleet message send` (self-sends are skipped), not into "the member's pane" generically. (3) `cafleet/tests/test_cli_member_ping.py`: added `test_unexpected_positional_argument_exits_two` to TestInputValidation, asserting that `cafleet member ping <extra>` exits 2 with Click's "got unexpected extra argument" wording — regression guard for the fixed-action contract (no operator-controlled keystroke body). |
| 2026-04-30 | Copilot review on PR #42 (round 3). Applied 1 fix. `skills/cafleet/roles/director.md` line 92 still carried the outdated `cafleet --session-id * member exec *` glob from before the round-2 sweep; updated to reference the actual `Bash(cafleet * member exec *)` rule (now described as "the operator's `permissions.ask` rule for `member exec` (currently `Bash(cafleet * member exec *)` in `.claude/settings.json`)") so all live mentions of the rule pattern match the canonical settings.json entry. |
| 2026-04-30 | Copilot review on PR #42 (round 4). Applied 1 fix. `docs/spec/cli-options.md` line 25 (`--session-id` global option row) listed required-subcommands as `member create/delete/list/capture/send-input/exec` but omitted `member ping`, contradicting the explicit enumeration on line 30. Added `ping` to the row's enumeration so the two lists agree. |
| 2026-04-30 | Copilot review on PR #42 (round 5). Body explicitly stated "Copilot reviewed 13 out of 13 changed files in this pull request and generated no comments" — Copilot has nothing further to add. Loop exited successful (no actionable feedback). Final commit on the branch is the round-4 fix; round 5 needed no source changes. |
