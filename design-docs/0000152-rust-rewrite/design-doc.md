...
| A8 | Capture anchoring | `capture_pane` on both backends over-fetches by a fixed 1000-line margin (fetch depth = requested lines + 1000, covering any realistic blank tail), drops the trailing run of visually-blank lines — whitespace-only after per-line CSI stripping, since TUI-painted empty rows carry ANSI sequences — and only then keeps the last N lines, so `--lines N` always returns up to N drawn lines anchored at the pane bottom regardless of the blank area under the cursor (user ruling after the Phase-D-era bug report; the Python reference's fetch-exactly-N blanks-passthrough is deliberately not preserved). The emitted content's ANSI handling, the `--json` shape (sha over the emitted content), and the no-trailing-newline emit are unchanged. |
