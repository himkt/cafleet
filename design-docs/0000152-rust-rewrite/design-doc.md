...
| A8 | Capture anchoring | `capture_pane` on both backends drops trailing visually-blank lines — whitespace-only after per-line CSI stripping, since TUI-painted empty rows carry ANSI sequences — from the fetched buffer before keeping the last N lines, so a small `--lines` window always shows the pane's drawn bottom instead of the blank area under the cursor on a tall pane (user ruling after the Phase-D-era bug report; the Python reference's blanks-passthrough is deliberately not preserved). Fetch argv, the emitted content's ANSI handling, the `--json` shape (sha over the emitted content), and the no-trailing-newline emit are unchanged. |
