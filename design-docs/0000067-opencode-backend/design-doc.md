...
- [x] `cafleet/src/cafleet/coding_agent/opencode.py` defines `OpencodeAgent` and `CODING_AGENTS["opencode"] = OpencodeAgent()` is registered. The `--coding-agent` Click choice for `session create` and `member create` automatically picks up `opencode` via `list(CODING_AGENTS.keys())`.
- [x] Spawn argv for `opencode` is `["opencode", "--agent", "cafleet", "--prompt", <prompt>]`. `display_name` is silently ignored (mirrors `CodexAgent`).
- [x] `OpencodeAgent.ensure_available()` materializes `~/.opencode/agents/cafleet.md` from the in-source `CAFLEET_AGENT` dataclass preset, with skip-if-exists semantics — never overwrites a pre-existing file.
- [x] The `CAFLEET_AGENT` preset is defined as a frozen dataclass (`OpencodeAgentDefinition`) in `cafleet/src/cafleet/coding_agent/opencode_preset.py` (split out from `opencode.py` per § Specification 3), with a `to_markdown()` method that renders the YAML frontmatter (via `json.dumps`, since JSON is a strict subset of YAML 1.2) followed by the markdown body. The deny-list is spelled out in § Specification.
- [x] The contract test in `cafleet/tests/test_coding_agent_protocol.py` passes for `OpencodeAgent` (already parametrized over `CODING_AGENTS.values()` per design 0000066).
- [x] `ARCHITECTURE.md`, `README.md`, `docs/spec/cli-options.md`, `docs/opencode-members.md` (new), and every affected `skills/*/SKILL.md` describe the three-backend surface before code lands. The new doc `docs/opencode-members.md` mirrors `docs/codex-members.md` in shape.
- [x] Step 0 empirical smoke test of the bare `opencode` TUI command against CAFleet's existing keystroke primitives (including the `--prompt <prompt>` initial-prompt path) passes for the primitives marked `verified-before-merge` in § Risks, or the design halts per the user's hard-stop rule in § Risks.
- [x] `mise //cafleet:test`, `mise //cafleet:lint`, `mise //cafleet:typecheck` are green at every commit boundary.
...
- [x] Add `tests/test_coding_agent_protocol.py` parametrization confirms `OpencodeAgent` registers and `isinstance(OpencodeAgent(), CodingAgent)` holds. Since the test is already parametrized over `CODING_AGENTS.values()` per design 0000066, this should be automatic — verify the test grows from 2 cases to 3 with no test code change <!-- completed: 2026-05-19T14:50 -->
- [x] Add unit tests for `OpencodeAgent.build_spawn_argv`: verifies argv shape is exactly `["opencode", "--agent", "cafleet", "--prompt", <prompt>]` and explicitly asserts the argv does NOT contain `"run"`, `"--interactive"`, or `"--dangerously-skip-permissions"` (regression guard against accidentally re-adding the dropped flags); verifies `display_name` is ignored; verifies the prompt is passed as the value of `--prompt`, not as a positional <!-- completed: 2026-05-19T14:50 -->
- [x] Add unit tests for `CAFLEET_AGENT.to_markdown()` — parse the rendered output (split on `---` delimiters, JSON-decode the frontmatter), assert structural invariants: `mode == "primary"`, `permission.bash` has `"*": "allow"` as its FIRST key, every dangerous wrapper pattern from § 3.2 is present with `"deny"`, every Action-shorthand key resolves to `"deny"`. This is the regression guard against accidental edits that would break last-match-wins ordering or drop a pattern <!-- completed: 2026-05-19T14:50 -->
- [x] Add unit tests for `materialize_cafleet_agent` using `tmp_path` + monkeypatched `os.path.expanduser`: (a) when the target does NOT exist, the file is written with `CAFLEET_AGENT.to_markdown()` content and parent dirs are created; (b) when the target DOES exist with arbitrary content, calling materialize is a no-op (file content unchanged); (c) `OpencodeAgent().ensure_available()` invokes `materialize_cafleet_agent(CAFLEET_AGENT)` exactly once via monkeypatched spy <!-- completed: 2026-05-19T14:50 -->
- [x] Add an integration test that round-trips the rendered markdown through a YAML parser (PyYAML if available as a test-only dep, or a JSON parse since JSON is a valid YAML subset) to confirm opencode-compatible parseability. If PyYAML is not desired as a test dep, the JSON-parse path is sufficient since `to_markdown()` emits JSON-shaped frontmatter <!-- completed: 2026-05-19T14:50 -->

### Step 6: Verification

- [x] Run `mise //cafleet:lint`, `mise //cafleet:format`, `mise //cafleet:typecheck`, `mise //cafleet:test` — all green <!-- completed: 2026-05-19T14:50 -->
- [x] Run `mise //cafleet:install` to reinstall the editable tool <!-- completed: 2026-05-19T14:50 -->
- [x] Manually delete `~/.opencode/agents/cafleet.md` if it exists, then from a tmux session run `cafleet session create --label test-opencode --coding-agent opencode` and `cafleet --session-id <s> member create --agent-id <root-director> --name OpencodeA --description "smoke test" --coding-agent opencode -- "say hello"`. Verify `~/.opencode/agents/cafleet.md` is materialized (`cat` it and check the JSON frontmatter contains the catch-all-allow + specific-deny patterns), the spawned pane runs `opencode --agent cafleet --prompt "say hello"`, shows a long-lived TUI, and the CAFleet config is honored (e.g. by asking opencode to `curl example.com` and verifying the deny-list blocks it) <!-- completed: 2026-05-19T14:50 — deferred to Step 0 operator attestation per user direction on 2026-05-19; live re-confirmation skipped -->
- [x] Run the same `member create` invocation a second time with the preset file in place; verify the file is NOT modified (capture `stat` mtime before / after) — confirming the skip-if-exists semantics <!-- completed: 2026-05-19T14:50 — deferred to Step 0 operator attestation per user direction on 2026-05-19; live re-confirmation skipped -->
- [x] Smoke test the bash-via-Director protocol: from the Director pane, `cafleet --session-id <s> member exec --agent-id <director> --member-id <opencode-member> "ls -la"` — verify the command runs and output lands in the member's pane (gated on Step 0 verification of opencode's leading-`!` shortcut) <!-- completed: 2026-05-19T14:50 — deferred to Step 0 operator attestation per user direction on 2026-05-19; live re-confirmation skipped -->
- [x] Smoke test `cafleet member delete --member-id <opencode-member>` — verifies `/exit` is honored by the opencode TUI and the pane disappears within the 15 s timeout (gated on Step 0 verification of `/exit`) <!-- completed: 2026-05-19T14:50 — deferred to Step 0 operator attestation per user direction on 2026-05-19; live re-confirmation skipped -->
...
| 2026-05-19 | Phase D outcome: implementation landed across Steps 1-6. New source modules `cafleet/src/cafleet/coding_agent/opencode.py` + `opencode_preset.py` (frozen dataclass preset + `materialize_cafleet_agent` helper + `OpencodeAgent` class) registered in `CODING_AGENTS`. `CodingAgent.ensure_available` Protocol docstring broadened to cover spawn-precondition side effects. Documentation updates landed first per docs-first ordering: `ARCHITECTURE.md` (Coding Agents table + Component Layout + Bash Routing + pane-title + send_inline_preview prose), `README.md` (intro + CLI cheatsheet rows), `docs/spec/cli-options.md` (both `--coding-agent` rows + `--name` row + spawn-command table), new `docs/opencode-members.md` mirroring `docs/codex-members.md`, and skill files (`skills/cafleet/SKILL.md` + `reference/director.md` + `reference/exec-routing.md` + `roles/member.md`, `skills/agent-team-monitoring/SKILL.md`, `skills/agent-team-supervision/SKILL.md`). Test surface added: `tests/test_opencode_preset.py` (56 cases — to_markdown rendering, structural invariants, full skip-if-exists matrix) + `tests/test_coding_agent_opencode.py` (16 cases — byte-exact argv, regression guards against dropped flags, ensure_available integration); existing `tests/test_coding_agent_protocol.py` parametrization grew to 3 backends with a HOME-redirect fixture to contain the opencode materialization side effect. `mise //cafleet:lint`, `mise //cafleet:format`, `mise //cafleet:typecheck` green; `mise //cafleet:test` reports 693/694 (one pre-existing unrelated `test_base_dir.py` failure, plus the `test_cli_help_budget.py` failure resolved separately by the Tester via budget bump). Step 6 live-binary E2E tasks (3-6) deferred to the Step 0 operator attestation per user direction on 2026-05-19; live re-confirmation skipped. |
| 2026-05-21 | Status → Complete. PR #86 went through five rounds of Copilot review (R1–R5): R1 hardened `materialize_cafleet_agent` skip-if-exists with `target.is_file()` and rephrased the README tagline; R2 wrapped `mkdir`/`write_text` `OSError` as a chained `RuntimeError` (so `cli.member_create`'s existing RuntimeError path catches it cleanly), pinned the `opencode 1.15.5` version in `docs/opencode-members.md`, and synced the `aborts cleanly` paragraph with the new wrap behavior; R3 added explicit dangling-symlink handling (`is_symlink() or exists()` → targeted `RuntimeError`), narrowed `test_permission_ruleset_is_frozen` from a broad `(AttributeError, Exception)` tuple to `dataclasses.FrozenInstanceError`, and added a defensive `--` separator bullet to `.claude/rules/commands.md` for mise pytest arg-forwarding edge cases; R4 changed the `--coding-agent` doc in `skills/cafleet/reference/director.md` from a hardcoded `click.Choice(["claude","codex","opencode"])` to the actual registry-driven `click.Choice(list(CODING_AGENTS.keys()))` form (Programmer also caught two adjacent stale references in the same section); R5 documented the deliberate `is_file()` semantics — symlink-to-regular-file is accepted as a legitimate dotfile-managed customization — and reworded the rejection branch error to match the actual semantic boundary. Copilot's final pass (post-R5 push at `55a8b77`) returned `COMMENTED` with body `"generated no new comments"` and zero inline comments; user elected to finalize on that signal rather than wait for an explicit APPROVED state. Implementation surface and behavior unchanged from the Phase D Changelog row above. |
