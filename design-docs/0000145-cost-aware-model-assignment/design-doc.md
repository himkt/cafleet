...
The legacy fixed-model policy tokens change meaning rather than disappear: each backend overlay's `{monitor_model}` / `{reviewer_model}` values mirror the model list's *Monitor and reviewer defaults* table and are updated with it on every refresh, so they are current defaults, not frozen policy. Every supervision, Director, monitor-role, and workflow example otherwise reflects the Director's pre-spawn choice from the model list; overlays carry backend facts (effort levels, permission flags) plus these mirrored defaults.
