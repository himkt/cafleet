...
- [x] A refresh skill updates the model list using only the supplied official Anthropic, OpenAI, Codex-availability, and OpenCode Zen documentation sources — with a dedicated procedure for the Zen-specific pitfalls (exact model IDs, the `opencode/` prefix, curated subset, limited-time free models) and the codex split (prices from the OpenAI pricing page, availability from the Codex models page) — requires maintainer review, and never silently changes a selection policy.
...
The model list is a catalog-style reference page read by the Director, not machine-parsed data: a preamble stating the maintenance rule, the 30-day freshness cadence, and the *last refreshed* date; a `Sources` section linking the approved official source pages; and one table per backend (`claude`, `codex`, `opencode`) with each model's spawn token, alias (claude), reviewed capability class, and standard input/output USD-per-MTok prices. The `opencode` table is a curated subset of the OpenCode Zen catalog whose model values are `opencode/<zen-model-id>`. Each backend's table is ordered most → least capable; capability classes and the ordering are reviewed maintainer judgment, and changing them requires reviewed policy approval in the same pull request. A *Monitor and reviewer defaults* table names each backend's current monitor and reviewer choice, re-derived from the backend tables on every refresh. Every listed model name and alias is a valid `--model` token; arbitrary pass-through strings remain manual-only.
...
The `{monitor_model}` / `{reviewer_model}` tokens change meaning rather than disappear: each backend overlay's values mirror the model list's *Monitor and reviewer defaults* table and are updated with it on every refresh, so they are current defaults, not frozen policy. Every supervision, Director, monitor-role, clean-docs, and workflow spawn instruction keeps its main-form token phrasing (`--role monitor --model {monitor_model}`, `--model {reviewer_model}`), resolved through the overlay as before; the selection policy — cost efficiency mode, the monitor/reviewer rules, overrides, fail-closed — lives in `roles/director.md` § *Model selection*, and the model list is catalog data only.
...
1. Fetch only the four user-approved official sources: `https://platform.claude.com/docs/en/about-claude/pricing.md` (claude), `https://developers.openai.com/api/docs/pricing` (codex prices), `https://learn.chatgpt.com/docs/models.md` (codex availability), and `https://opencode.ai/docs/zen.md` (opencode). Do not use search results, third-party price sites, or social posts as model-list authority.
...
| 2026-07-20 | Post-approval revision 10 (user directive): `{monitor_model}` / `{reviewer_model}` return to every backend overlay as values mirroring the model list's defaults table (kept in sync by the refresh skill), restoring main's placeholder-example cells in the clean-docs and team-skill required-reading tables; the curated-Zen-subset / manual-spawn instruction moves from `reference/director.md` § *Model list* to the model list's `opencode` section. |
| 2026-07-20 | Post-approval revision 11 (user directive): sources switch to the raw-markdown endpoints (`pricing.md`, `zen.md`) and `https://learn.chatgpt.com/docs/models.md` joins as the codex **availability** source — codex prices come from the OpenAI pricing page, but the table carries only the models Codex currently offers in its picker; a model absent there or demoted to a legacy model (reachable only via `codex -m`) is removed even if still priced. `gpt-5.4-mini` and `gpt-5.4` are removed on that basis (verified against the Codex v0.144.1 picker) and the codex monitor default becomes `gpt-5.6-luna`. |
| 2026-07-20 | Post-approval revision 12 (user directive): every skill spawn instruction returns to the main-form token phrasing (`--role monitor --model {monitor_model}`, `--model {reviewer_model}`) — the supervision, monitor-role, cafleet/team SKILL.md, clean-docs, and workflow pages are restored to main, along with the `{monitor_model}`/`{reviewer_model}` documented defaults (inherit the parent) in *Resolve your overlay*; the selection policy stays canonical in `roles/director.md` § *Model selection* and the model list drops its policy prose to remain catalog data only. |
