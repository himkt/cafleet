...
TEAM COMPOSITION NOTE: This is a documentation-only design doc — no Tester was spawned and there are no unit tests to locate. Skip the locate-the-tests step of Phase 2; for each assigned step, read the step spec at the pointer, apply the documentation edits, and verify per the design doc's Step 5 commands where applicable.
