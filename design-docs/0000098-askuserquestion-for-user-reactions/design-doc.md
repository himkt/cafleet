...
- [x] A single normative section — `Soliciting user reactions (AskUserQuestion)` — exists in `skills/cafleet/SKILL.md`, owning the principle, the question-shape taxonomy, the exemptions, and the codex/opencode backend caveat.
- [x] The backend caveat is stated **once** (in the canonical section); the `cafleet-agent-team-monitoring` instance is reduced to a pointer with no duplicated normative text.
- [x] Every surface-less escalation in `skills/` (7 sites) names `AskUserQuestion` with concrete continue/abort options.
- [x] No new divergent copy of the question-shape taxonomy: the `cafleet-agent-team-supervision` User Delegation Protocol references the canonical taxonomy instead of restating it.
...
- [x] `README.md` and `docs/concepts/` need no change (verified: they carry no user-reaction norm; the only `docs/` AskUserQuestion mentions are `member send-input` CLI mechanics, already correct).
...
- [ ] Regenerate the installed copies with `mise //:skill-install`. <!-- completed: --> (operator-run local propagation; see Changelog)

---

## Changelog

- 2026-06-18 — Implemented, reviewed, and shipped on `feat/0000098-askuserquestion-for-user-reactions` (PR #128). An independent reviewer gave an unconditional approval (round 3); `mise //cafleet:lint` and the surface-less-escalation grep gate both pass; a Copilot review pass was also addressed. The sole remaining task is the operator-run `mise //:skill-install` propagation (Step 6 third task / Success Criterion 5), which regenerates the `~/.claude/skills` install artifacts and by design cannot appear in the PR diff — run it locally to complete the rollout.
