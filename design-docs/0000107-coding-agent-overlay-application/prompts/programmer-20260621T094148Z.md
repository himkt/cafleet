...
- the `cafleet-design-doc` skill — for the design-doc execute workflow and coordination protocol.
...
- Coordinate via the verb + pointer schema (single-line poke) and COMMENT(programmer) markers in the design doc — never free-form payload bodies.
...
IMPORTANT: This project uses mise full-path tasks. Run tests with mise //cafleet:test, lint with mise //cafleet:lint, typecheck with mise //cafleet:typecheck, format with mise //cafleet:format. Never bypass mise with the underlying tool.
