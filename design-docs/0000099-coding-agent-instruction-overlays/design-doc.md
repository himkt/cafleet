...
- [x] Three overlay files exist at `skills/cafleet/reference/coding-agent/{claude,codex,opencode}.md`, each a compact `| Placeholder | Value |` table defining the 8 canonical placeholders (§2) for its backend.
- [x] An overlay template exists at `skills/cafleet/reference/coding-agent/_template.md`, carrying the 8-placeholder value-table skeleton with angle-bracket fill-in guidance and no backend-specific values (§5a). The three overlays follow its table structure.
...
Every value below moves OUT of the base and into the overlays. The base carries only a `{placeholder}` token — delta 1 → `{decision_surface}`; 2 → `{monitor_model}` (an ordinary member's example `--model` value comes from the inference table, not a placeholder); 3 → `{permission_flags}`; 4 → `{bg_run}` / `{bg_stop}` / `{task_coord}`; 5 → `{pane_title}`; 6 → `{skill_loader}`. The three overlay columns give each backend's value for that token (the overlay's value table). The "base intent" column states what the neutral instruction conveys — the prose the `{placeholder}` stands in for.
...
- Carries the `| Placeholder | Value |` table with all 8 canonical placeholder rows (§2), each value an angle-bracket guidance description of what the backend states there (no backend-specific values).
...
Every overlay's table must define all 8 canonical placeholders; a backend with "no analog" for a value states that *as* the value (e.g. `{task_coord}` = "none — coordinate via cafleet messages") rather than omitting the row.
...
- [x] Create `skills/cafleet/reference/coding-agent/claude.md` covering all six deltas for claude per §4 (the value table later refined to clean noun-phrase values + a constraints/taxonomy note line — see §5/§5a and the Changelog). <!-- completed: 2026-06-19T11:09 -->
...
| 2026-06-19 | Two-reviewer round (correctness Reviewer + Simplicity Reviewer) on the redesign: **cut `{member_model}` (9 → 8 placeholders)** — redundant with the inference table; shortened the codex/opencode `{decision_surface}` / `{task_coord}` values to clean noun phrases with caveats moved to per-overlay note lines (fixes inline-substitution defects: doubled tokens, nested/colliding parentheticals, number disagreement); **restored a compact question-shape taxonomy** in `claude.md`'s note line (the base pointed at it but the 74→18 compression had dropped it); trimmed `{pane_title}`; fixed the `{bg_stop}` frame to use a backend-neutral process handle instead of a claude task id. |
| 2026-06-20 | Both reviewers **approved** the redesign (correctness + simplicity); toolchain green (863 tests); pushed to PR #130. Status → Complete. (Programmer 361 was retired at context exhaustion and replaced by Programmer2 365 mid-round.) |
