...
Wait for the Director to assign a document for review (cafleet body: ready (doc)). When you receive that message, the doc pointer refers to the DESIGN DOCUMENT path above — read that file and provide specific, actionable feedback per the role definition. Record each finding as a COMMENT(reviewer): [TAG] <body> marker inline at the affected section, and report complete (doc) — N issues (or approved (doc)) to the Director.
