...
**Phase 1 (Tester Test-Framework Selection)** is brought into the schema. If the framework is deterministically detected, the Tester proceeds silently to Phase 2 — no cafleet message. If the framework is ambiguous, the Tester writes `COMMENT(tester): framework selection ambiguous — found <evidence>; need user arbitration` at `paragraph-Implementation` and sends `blocked (paragraph-Implementation)` — the cafleet pointer matches the marker location per the protocol invariant. The Director relays via `AskUserQuestion`, writes the answer back as `COMMENT(claude): <choice>` at the same paragraph, sends `ready (paragraph-Implementation)`, and the Tester resumes. (If the design doc has no `## Implementation` heading at the time of selection, the Tester falls back to `paragraph-Overview` and the cafleet body becomes `blocked (paragraph-Overview)` — same pairing rule.)
