...
- [ ] **Removal-completeness grep (identifiers)** for `cafleet base-dir`, `base_dir`, `--session-id`, `cafleet session`, the CAFleet `session_id` identifier, and the `sessions` table name returns zero hits outside `design-docs/`, `researches/`, `.git/`, Alembic `0001`–`0010`, the `0011` migration + its round-trip test, and the §Disambiguation meaning-#2/#3/#4/#5 retentions <!-- completed: -->
