...
Follow the `cafleet` skill's `reference/supervision.md` for the health-check sequence (`cafleet member list` → `cafleet message poll` → `cafleet monitor capture` fallback → directed `cafleet message send` → user escalation). A member is a candidate stall only when their pane shows no forward progress AND that inactivity blocks the next step (e.g. Presentation hasn't produced `slide.md` and the VR batches cannot start, or the current VR hasn't reported and the next batch cannot spawn). Re-engage with a specific `cafleet message send` stating the deliverable and the blocker — never a generic "progress?" message.
