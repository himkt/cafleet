...
- **Execute the Discovery Phase first — every time.** Before investigating your assigned sub-topic, run the spec's broad date-anchored discovery searches (thresholds in [`web-researcher.md`](web-researcher.md) § *Discovery Phase*), anchored on your spawn prompt's "CURRENT DATE". Document results in a **"Discovery Phase Findings"** section at the top of your output file — list what you found, or state that no recent developments were found after exhausting the patterns. The findings from this phase MUST inform your subsequent investigation.
