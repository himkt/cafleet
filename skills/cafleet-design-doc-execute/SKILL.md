...
- **`$ARGUMENTS` absent** (the discover-all-approved-docs flow): the no-argument form scans `<repo-root>/design-docs/`, so the Director MUST invoke from the repo root. Verify with `git rev-parse --show-toplevel` and abort with a clear "invoke from the repo root" error if `cwd` differs. Since `cwd` is the verified repo root, pin `${BASE}` to it directly for this branch:

  Do NOT run the `cafleet-base-dir` shared-root resolution (Step 1 / Step 2) here. In the edge case where the repo root is itself `$HOME` or under `~/.claude`, shared-root resolution would drive **Step 2** `AskUserQuestion` and could select `/tmp/claude-code` — making `${RESOLVED_ARGS} = /tmp/claude-code/design-docs/` and pointing the discovery scan at the wrong directory. With `${BASE}` pinned to the verified repo root, set `${RESOLVED_ARGS} = ${BASE}/design-docs/` — this matches Tier 3 below and engages the discovery flow that scans every approved slug under `<repo>/design-docs/`.
