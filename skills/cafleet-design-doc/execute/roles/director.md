...
- **Bootstrap the CAFleet fleet (monitor included) and keep an active heartbeat.** Load the `cafleet` skill and Read its `reference/supervision.md`; follow its § *Spawn Protocol* end to end — fleet bootstrap via `cafleet fleet create`, the `monitor live` gate on the first ordinary `member create`, and dispatch-on-ready. Keep the monitor member alive until shutdown deletes it first-out.
...
- **Drive every task to completion** per `execute.md` § *Run to completion* — dispatch the next task to each idle member as soon as it is ready; the designed gates (Step 6 user approval, "stop means stop", new-decision escalations) remain.
...
- **Verify Success Criteria before user approval.** Mandatory — per `execute.md` Step 5's gate: verify each criterion and check it off before spawning the Reviewer.
...
- **Run the Reviewer review loop after all tasks finish (Step 5).** Spawn the fresh Reviewer (`--model {reviewer_model}`, the first and only time it exists in the fleet) and drive the review-and-revise loop per `execute.md` Step 5 — no round cap; it ends only on `approved (doc)` or an explicit user halt/abort ("stop means stop").
- **Push & PR after admin approval.** On Approve, run Steps 7 → 8 without further prompting (procedure + fallbacks in `execute.md`); a precondition failure or approve-local intent skips Step 7 → Step 8 local-finalize.
- **Clean up when done.** Final commit updating status to "Complete", then the § Shutdown Protocol below.
...
Every Director action below is a re-sent stall-nudge — the recipient interprets it contextually per [../../reference/coordination.md](../../reference/coordination.md): same target, same expected action.
...
Run the canonical teardown per the `cafleet` skill's `reference/supervision.md` § *Cleanup Protocol* (the monitor member is deleted first, first-out).
