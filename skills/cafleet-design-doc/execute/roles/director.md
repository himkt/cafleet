...
| Reviewer Review (Step 5) | Reviewer reports `complete (doc) — N issues` or `approved (doc)` | Reviewer goes idle without a report | `cafleet message send --fleet-id <fleet-id> --agent-id <director-agent-id> --to <reviewer-agent-id> --text "ready (doc)"` (re-sent stall-nudge — same target, same expected action) |
