...
If a queued action requires a *new* decision the user has not yet made (choosing between options, approving a risky / remote-visible operation, disambiguating a teammate's question), use your decision surface (the canonical user-reaction rule is the `cafleet` skill § *Soliciting user reactions*; the concrete surface is in your overlay) — do **not** emit a passive hold and wait. The hold message produces nothing; the question unblocks you within seconds and produces a recorded answer.

## Spawn Protocol

**Spawn order (first-in): the monitoring member comes first.** The **first** `cafleet member create` in the fleet IS the dedicated monitoring member (`--role monitor --model <cheapest capable model for the monitor's backend>`); it starts the monitor and gates every ordinary `member create` behind its `ready: monitor live` handshake. The Director never runs `cafleet monitor start` itself. See the `cafleet-agent-team-monitoring` skill § The monitoring member for the canonical spawn prompt.
...
1. **Verify env, then ensure supervision is running**:
   ...
   - **Monitoring member up + monitor live before any ordinary member** — the spawn-gate is canonical in the `cafleet-agent-team-monitoring` skill § The monitoring member (the first `member create` is `--role monitor --model <cheapest capable model for the monitor's backend>`; its `ready: monitor live` handshake gates the first ordinary `member create`; wait on the message, do not block-poll status).
...
CAFleet members never talk to the user directly — the Director relays. This is the relay-specific application of the canonical rule in the `cafleet` skill § *Soliciting user reactions* (the concrete decision surface and question-shape taxonomy are in your overlay). When a member sends a `cafleet message send` asking for user input:

1. **Classify the question shape** per the question-shape taxonomy in your overlay (choice among labeled options, approve / yes-no, continue-or-abort, or open-ended / draft selection), and mirror it into your decision surface's options. Follow your overlay for how the surface handles free-form text.
...
| Start the supervision tick | Spawn the monitoring member first: `cafleet member create --fleet-id <s> --agent-id <director> --name monitor --description <…> --role monitor --model <cheapest capable model for the monitor's backend> --prompt-file <…>`; it runs `cafleet monitor start` in its own pane — see the `cafleet-agent-team-monitoring` skill | Its `ready: monitor live` handshake gates the first ordinary `member create`. |
...
| Answer a member's relayed question | the decision-relay primitive your overlay describes (`../cafleet/reference/coding-agent/<name>.md`) | Delegate the decision to the user via your decision surface first; never decide silently |
| Relay user input | your decision surface → `cafleet message send` | Pass-through; never substitute judgment |
