...
| Start the supervision tick | Spawn the monitoring member first: `cafleet --fleet-id <s> member create --agent-id <director> --role monitor --model sonnet --prompt-file <…>`; it runs `cafleet monitor start` in its own pane — see the `cafleet-agent-team-monitoring` skill | Its `ready: monitor live` handshake gates the first ordinary `member create`. |
