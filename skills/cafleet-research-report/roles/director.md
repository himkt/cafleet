...
- **Clean up when done.** Follow the Shutdown Protocol in the `cafleet` skill: stop the monitor's background task (there is no `monitor stop` command), run `cafleet member delete` per member (Researchers, then Scouts, then Manager), verify the roster is empty with `cafleet member list`, then `cafleet fleet delete [fleet-id]`.
