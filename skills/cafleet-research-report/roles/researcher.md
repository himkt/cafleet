...
- **Claim your task on start.** Your spawn prompt includes a `YOUR TASK ID`. Claim your assignment via {task_coord} as your first action (on a harness task list, set owner `researcher-NN`, status `in_progress`) — substitute `NN` with the literal two-digit number from the `researcher-NN` name your spawn prompt assigned (e.g., `researcher-01`). The `owner` value is the concrete name string, not a token containing brackets. Mark it `completed` when the output file is written and your completion report has been sent.
