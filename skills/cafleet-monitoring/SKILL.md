...
> **Bash request blocking case**: When `cafleet message poll` returns a member message asking for a shell command, dispatch `! <command>` via `cafleet member send-input` per `Skill(cafleet)` § Routing Bash via the Director. Member blocks until the keystroke lands; process requests one at a time, don't skip ahead to other inbox items.
...
| Process pending shell-command request from member | Blocking on member side | Dispatch `! <command>` via `cafleet member send-input` per `Skill(cafleet)` § Routing Bash via the Director. Don't skip past a member's request — the member sits idle until the keystroke lands. |
