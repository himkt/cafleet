...
> **Bash request blocking case**: When the Director's `cafleet message poll` output contains a `bash_request` JSON payload (a `text` body that parses as `{"type": "bash_request", ...}`), load `Skill(cafleet)` § Routing Bash via the Director and follow the 6-step dispatch BEFORE any other inbox item. The member is blocked waiting for the `bash_result` reply (no member-side timeout); skipping past the request leaves the member idle indefinitely. This bridges the case where a Director loaded only `Skill(cafleet-monitoring)` at session start and discovers a bash_request later.
...
| Process pending `bash_request` from member | Blocking on member side | When `cafleet message poll` shows an unresponded `bash_request` JSON payload from a member, the Director MUST process it BEFORE any other inbox item — the member is blocked waiting for the `bash_result` reply (no member-side timeout). Follow the 6-step dispatch in `Skill(cafleet)` § Routing Bash via the Director: discriminate the payload, match against `permissions.allow`, AskUserQuestion-on-miss, run via `cafleet member exec`, reply with a `bash_result` via `cafleet message send`. Do NOT skip past a `bash_request` — the member will sit idle indefinitely. |
