...
| Process pending shell-command request from member | Blocking on member side | When `cafleet message poll` shows an unresponded member message asking for a shell command, the Director MUST respond promptly via `cafleet member send-input --member-id <m> --freetext "! <command>"` (Claude Code's `!` CLI shortcut runs it natively in the member's pane). See `Skill(cafleet)` § Routing Bash via the Director for the full convention. Do NOT skip past a member's shell-command request — the member will sit idle. |
