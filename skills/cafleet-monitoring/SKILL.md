...
> **Bash request blocking case**: When the Director's `cafleet message poll` output contains a member message asking to run a shell command (plain text like "Please run `git log` for me"), the Director MUST respond promptly via `cafleet --session-id <session-id> member send-input --agent-id <director-agent-id> --member-id <member-agent-id> --freetext "! <command>"`. The member is blocked until the `!` keystroke lands and Claude Code's CLI runs the command. Process command requests in strict poll order. Don't skip ahead to other inbox items — finish each `! cmd` send-input dispatch before moving on. The blocked member is waiting for its `!` keystroke. See `Skill(cafleet)` § Routing Bash via the Director for details.
...
| Process pending shell-command request from member | Blocking on member side | When `cafleet message poll` shows an unresponded member message asking for a shell command, the Director MUST respond promptly via `cafleet --session-id <session-id> member send-input --agent-id <director-agent-id> --member-id <member-agent-id> --freetext "! <command>"` (Claude Code's `!` CLI shortcut runs it natively in the member's pane). See `Skill(cafleet)` § Routing Bash via the Director for the full convention. Do NOT skip past a member's shell-command request — the member will sit idle. |
