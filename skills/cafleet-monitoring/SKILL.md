...
> **Bash request blocking case**: When `cafleet message poll` returns a member message asking for a shell command, dispatch using the smart-routing rule: try `cafleet member safe-exec --agent-id <director-agent-id> --member-id <member-agent-id> "<cmd>"` first; on exit 3 (ask) fall back to `cafleet member exec --agent-id <director-agent-id> --member-id <member-agent-id> "<cmd>"`. See `Skill(cafleet)` § Routing Bash via the Director. `safe-exec` re-reads the operator's `permissions.allow` / `permissions.deny` rules and exits 0 (allow, keystroke sent), 2 (deny — relay rejection, do not retry), or 3 (ask — proceed to `exec` fallback). `exec` is bare dispatch and surfaces the operator's outer `permissions.ask` prompt. Member blocks until the decision lands; process requests one at a time, don't skip ahead to other inbox items.
...
| `cafleet ... message send --agent-id <director-agent-id> --to <member-agent-id> --text "..."` | Interactive, authoritative | Third -- send a specific instruction to unstick the member (push notification triggers the member's pane to poll) |
| `cafleet ... member send-input --agent-id <director-agent-id> --member-id <member-agent-id> (--choice N \| --freetext "<text>")` | Interactive, restricted keystroke | Answers an `AskUserQuestion`-shaped prompt — delegate the decision to the user via the Director's own `AskUserQuestion` tool call FIRST, then invoke the resolved command via the Director's Bash tool (Claude Code's native per-call permission prompt is the consent surface; never print a fenced `bash` block for the user to paste). See the cafleet skill's "Answer a member's AskUserQuestion prompt" section for the canonical three-beat workflow + pane-shapes table. Same authorization boundary as `capture`. |
| `cafleet ... member safe-exec --agent-id <director-agent-id> --member-id <member-agent-id> "<cmd>"` | Interactive, permission-aware bash dispatch | Smart-routing first stage: routes a shell command via Claude Code's `!` keystroke after re-reading the operator's three-layer `permissions.allow` / `permissions.deny` rules. Tri-state: exit 0 allow (keystroke sent), exit 2 deny (named pattern + source file in stderr — relay rejection, do not retry), exit 3 ask (fall back to `member exec`). See `Skill(cafleet)` § Routing Bash via the Director. Same authorization boundary as `capture`. |
| `cafleet ... member exec --agent-id <director-agent-id> --member-id <member-agent-id> "<cmd>"` | Interactive, operator-prompted bash dispatch | Smart-routing fallback (only invoked when `safe-exec` exited 3): bare dispatch with no internal permission decision. Surfaces the operator's outer `permissions.ask` prompt (configured for `Bash(cafleet --session-id * member exec *)`) and on accept keystrokes `! <cmd>` into the member pane. See `Skill(cafleet)` § Routing Bash via the Director. Same authorization boundary as `capture`. |
...
| Process pending shell-command request from member | Blocking on member side | Dispatch using the smart-routing rule: try `cafleet member safe-exec ... "<cmd>"` first, fall back to `cafleet member exec ... "<cmd>"` on exit 3 (ask). See `Skill(cafleet)` § Routing Bash via the Director. Don't skip past a member's request — the member sits idle until the decision lands. |
...
4. If a member's terminal shows no forward progress since last check, send them a specific instruction via `cafleet --session-id <session-id> message send --agent-id <director-agent-id> --to <member-agent-id> --text "Report your progress now. If blocked, state what is blocking you."` (the push notification will trigger the member's pane to poll and pick up the message).
