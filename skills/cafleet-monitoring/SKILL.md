...
> **Bash request blocking case**: When `cafleet message poll` returns a member message asking for a shell command, dispatch via `cafleet member safe-exec --bash "<cmd>"` per `Skill(cafleet)` § Routing Bash via the Director. The CLI re-reads the operator's `permissions.allow` / `permissions.deny` rules and exits 0 (allow, keystroke sent), 2 (deny — relay rejection), or 3 (ask — relay suggested `Bash(...)` pattern). Member blocks until the decision lands; process requests one at a time, don't skip ahead to other inbox items.
...
| `cafleet ... member safe-exec --agent-id <director-agent-id> --member-id <member-agent-id> --bash "<cmd>"` | Interactive, permission-aware bash dispatch | Routes a shell command via Claude Code's `!` keystroke after re-reading the operator's three-layer `permissions.allow` / `permissions.deny` rules. Tri-state: exit 0 allow (keystroke sent), exit 2 deny (named pattern + source file in stderr), exit 3 ask (relay the suggested `Bash(...)` pattern to the operator). See `Skill(cafleet)` § Routing Bash via the Director. Same authorization boundary as `capture`. |
...
| Process pending shell-command request from member | Blocking on member side | Dispatch via `cafleet member safe-exec --bash "<cmd>"` per `Skill(cafleet)` § Routing Bash via the Director. Don't skip past a member's request — the member sits idle until the decision lands. |
