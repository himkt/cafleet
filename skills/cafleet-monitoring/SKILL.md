...
> **Bash request blocking case**: When `cafleet message poll` returns a member message asking for a shell command, dispatch via `cafleet member send-input --bash "<cmd>"` per `Skill(cafleet)` § Routing Bash via the Director. Member blocks until the keystroke lands; process requests one at a time, don't skip ahead to other inbox items.
...
| `cafleet ... member send-input --agent-id <director-agent-id> --member-id <member-agent-id> (--choice N \| --freetext "<text>" \| --bash "<cmd>")` | Interactive, restricted keystroke | `--choice` / `--freetext` answer an `AskUserQuestion`-shaped prompt — delegate the decision to the user via the Director's own `AskUserQuestion` tool call FIRST, then invoke the resolved command via the Director's Bash tool (Claude Code's native per-call permission prompt is the consent surface; never print a fenced `bash` block for the user to paste). `--bash` routes a shell command via Claude Code's `!` keystroke (no AskUserQuestion gate). See the cafleet skill's "Answer a member's AskUserQuestion prompt" section for the canonical three-beat workflow + pane-shapes table, and § Routing Bash via the Director for the `--bash` flow. Same authorization boundary as `capture`. |
...
| Process pending shell-command request from member | Blocking on member side | Dispatch via `cafleet member send-input --bash "<cmd>"` per `Skill(cafleet)` § Routing Bash via the Director. Don't skip past a member's request — the member sits idle until the keystroke lands. |
