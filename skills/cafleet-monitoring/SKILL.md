...
| Process pending `bash_request` from member | Blocking on member side | When `cafleet poll` shows an unresponded `bash_request` JSON payload from a member, the Director MUST process it BEFORE any other inbox item — the member is blocked waiting for the `bash_result` reply (no member-side timeout). Follow the 6-step dispatch in `Skill(cafleet)` § Routing Bash via the Director: discriminate the payload, match against `permissions.allow`, AskUserQuestion-on-miss, run via `cafleet bash-exec`, reply with a `bash_result` via `cafleet send`. Do NOT skip past a `bash_request` — the member will sit idle indefinitely. |
