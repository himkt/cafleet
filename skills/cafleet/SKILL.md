...
Run `cafleet member ping` after any `cafleet member exec` invocation that exits 0. Skip the ping only on non-zero exit — the dispatch never reached the pane, and the 1-minute `cafleet-monitoring` tick is the safety net.
