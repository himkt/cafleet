...
Use literal ids, never shell variables — `permissions.allow` matches Bash invocations as fixed strings, so `$VAR` expansion breaks the match and forces prompts. The full rationale and the `permissions.allow` coverage rule are in [`cli-options.md`](../../docs/spec/cli-options.md#fleet-id).
