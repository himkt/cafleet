...
When a Director spawns a team, the **FIRST** member created is the dedicated monitoring member (`cafleet member create --role monitor --coding-agent <chosen backend> --model <chosen model> --text-file <rendered monitor prompt>`, where the Director chooses the backend/model pair from the model list — see [`reference/director.md`](reference/director.md) § *Model selection before member create*). It owns the heartbeat and gates every ordinary `member create` behind its `ready: monitor live` handshake. The Director never runs `cafleet monitor start` itself.
