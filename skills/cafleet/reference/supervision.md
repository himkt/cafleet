...
CAFleet members do not act autonomously. The team's periodic heartbeat is hosted by the fleet's dedicated **monitor member** — a cheap-model watcher you spawn FIRST (`cafleet member create --role monitor`, § *Spawn Protocol*). At startup the monitor member launches **`cafleet monitor`**, a per-fleet `scan → wake → sleep` loop, as a background task in its own pane — and it is the **only party that does so**: you never run the loop yourself — you spawn the monitor member and wait for `monitor live` — and ordinary members never run it. Because the loop is just a backgrounded command, the heartbeat is **backend-agnostic** — a monitor member on `claude`, `codex`, or `opencode` gets the identical tick.
