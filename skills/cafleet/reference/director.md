...
| `--role` | no | Sole accepted value `monitor` — registers the fleet's **monitor member**. Recovery-only: the bootstrap monitor is spawned by `cafleet fleet create`; use this flag solely to re-spawn a dead monitor mid-run (`--model {monitor_model}`, omit `--coding-agent`; protocol in [`roles/monitor.md`](../roles/monitor.md)). One active monitor member per fleet, and an ordinary `member create` requires one — both CLI guard error strings are in [`cli-options.md`](../../../docs/docs/spec/cli-options.md#error-messages). Any other value is the parser's invalid-value error (exit 2). |
...
**Long or multi-line message bodies**: the same `ARG_MAX` cliff applies to `message send` / `message broadcast` — a long or multi-line body MUST be passed via `--file <path>` (or `--file -` for stdin), never as the inline positional. This rule is canonical in [`reference/supervision.md`](supervision.md) § Communication Model.
