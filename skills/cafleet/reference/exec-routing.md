...
The bash-via-Director protocol is the **fallback** for the harness deny-list. Members spawn with workspace-scoped auto-approval enabled — `claude` uses `--permission-mode dontAsk`, `codex` uses `--ask-for-approval never --sandbox workspace-write` — so the Bash tool is enabled and routine permission prompts auto-resolve silently. Members run cafleet (and any other shell command) directly via the Bash tool by default. **No prefix, no Director routing required by default.**

The fallback fires only when the coding-agent harness deny-list rejects a Bash invocation (e.g. `git push`, `rm -rf`). In that case the member auto-routes: it sends a plain CAFleet message to its Director asking for the command, and the Director dispatches the command into the member's pane via `cafleet member exec`, which keystrokes literal `! <cmd>` + `Enter` and triggers the coding agent's `!` CLI shortcut on the receiving side (honored by both `claude` and `codex`). No new broker primitives, no extra helper machinery — just the existing message-passing + tmux-keystroke infrastructure plus the dedicated `member exec` subcommand.
