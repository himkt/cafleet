...
Before assuming a member is stalled, run the cheap checks first: poll your own inbox (`cafleet message poll` — the member may have replied via a keystroke you missed) without touching its pane, then capture the member's pane (`cafleet member capture`, default `--lines 30`; bump to `--lines 120`/`200` to show a member's full decision-prompt frame). The same poll→capture detection drives the supervision tick — see the `cafleet-agent-team-monitoring` skill § Stall Response.
