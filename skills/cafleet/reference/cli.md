...
Three backends — `claude`, `codex`, `opencode` — chosen per member at `member create` time via `--coding-agent` (omitted → the member inherits the Director's backend). `--model <m>` pins the LLM, and `--effort <level>` forwards a reasoning-effort level (claude: `low`–`max`; codex: `minimal`–`xhigh`; opencode: unsupported — any value exits 2). `--role monitor` re-spawns the fleet's monitor member mid-run (the bootstrap monitor is spawned by `fleet create`; one active per fleet). The flags, the model-name-to-backend inference, and the spawn-argv detail live in [`reference/director.md`](director.md). Read the selected member backend's Model catalog, Role defaults, and runtime capability tables in [`coding-agents.md`](coding-agents.md) for model and effort selection. All three honor the leading-`!` input shortcut, so `member prompt --shell` and inline previews work uniformly. Per-backend deltas: [`claude`](coding-agents.md#claude) / [`codex`](coding-agents.md#codex) / [`opencode`](coding-agents.md#opencode).
