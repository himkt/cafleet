...
Drafter and Reviewer members are spawned with `--permission-mode dontAsk` (Bash tool enabled, permission prompts auto-resolve), so they run shell commands directly by default. The bash-via-Director protocol is the fallback when a member's Bash invocation is rejected by the Claude Code harness deny-list (destructive operations such as `git push`). In that case the member auto-routes by sending a plain shell-command request via `cafleet message send`, and the Director responds by dispatching `cafleet member safe-exec --bash <command>` — the CLI re-reads the operator's `permissions.allow` / `permissions.deny` rules and either keystrokes `! <command>` into the member pane (exit 0 allow), reports the matched deny pattern (exit 2 deny), or surfaces a suggested `Bash(...)` pattern for the operator to add (exit 3 ask). Process such requests one at a time in poll order. Full invocation + flag layout in `Skill(cafleet)` § Routing Bash via the Director.
