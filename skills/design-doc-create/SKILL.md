...
2. **Write the audit file** to `<base-dir>/drafter.md` (normal mode) or `<base-dir>/drafter-resume.md` (resume mode), where `<base-dir>` is resolved by `Skill(cafleet:base-dir)` in Step 0. If `Skill(cafleet:base-dir)` was skipped (absolute-path argument), `<base-dir>` is undefined and the audit-file write is skipped per the design doc *Audit File Layout* gating rule. Overwrites on subsequent spawns of the same role-type within this invocation; that is intentional.
...
2. **Write the audit file** to `<base-dir>/reviewer.md` (`<base-dir>` resolved by `Skill(cafleet:base-dir)` in Step 0). If `Skill(cafleet:base-dir)` was skipped (absolute-path argument), `<base-dir>` is undefined and the audit-file write is skipped per the design doc *Audit File Layout* gating rule. Overwrites on subsequent spawns of the same role-type within this invocation; that is intentional.
