...
def send_bash_command(*, target_pane_id: str, command: str) -> None:
    ...
    Used by ``cafleet member exec`` and ``cafleet member safe-exec`` to
    route shell commands via Claude Code's ``!`` shortcut. Unlike
    ``send_freetext_and_submit``, there is NO leading ``4`` keystroke
    (no AskUserQuestion gate).
    """
