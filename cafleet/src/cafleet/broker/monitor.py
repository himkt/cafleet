...
def update_monitor_config(
    ...
    *,
    ...
    accepted as a bool and written as 0/1; disabling clears
    ``last_stall_check_at``.
...
def reconcile_monitor_lifecycle(
    ...
    Clears ``last_stall_check_at`` so a member that later re-enables or
    rebinds a pane starts its stall-check cadence fresh.
    """
    ...
    with _shared.write_session() as session:
        session.execute(
            ...
            .values(last_stall_check_at=None)
        )
...
def list_monitor_targets(fleet_id: int) -> list[dict]:
    ...
    wake-entry role label), ``pane_id``, ``interval_seconds``,
...
def clear_monitor_runtime(fleet_id: int, pid: int) -> None:
    ...
    A cleanly-stopped monitor leaves no residual ``started_at``, so the WebUI
    reports a fully-stopped row. A non-owner clear matches zero rows and is
...
def monitor_runtime_payload(fleet_id: int, now: datetime) -> dict:
    """Build the runtime-liveness dict consumed by the WebUI ``GET /api/monitor``.
...
def monitor_members_payload(fleet_id: int, now: datetime) -> list[dict]:
    """Build the per-member rows consumed by the WebUI ``GET /api/monitor``.

    One dict per ``list_monitor_targets`` row. ``last_ping_age_seconds`` and
