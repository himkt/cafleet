...
def message_send(ctx, from_member_id, to_member_id, text, text_file, full, quiet):
...
def message_broadcast(ctx, from_member_id, text, text_file, full):
...
def message_poll(ctx, member_id, full):
...
def message_ack(ctx, member_id, task_id, full, quiet):
...
def message_cancel(ctx, member_id, task_id, full):
...
def message_show(ctx, member_id, task_id, full):
