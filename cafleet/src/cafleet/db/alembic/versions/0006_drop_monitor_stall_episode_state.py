"""drop monitor stall episode state

Revision ID: 0006
Revises: 0005
Create Date: 2026-07-30 08:15:20.082329
...
def upgrade() -> None:
    ...
    # SQLite cannot drop table-level CHECK constraints in place, so this is a
    # batch recreate of monitor_config; it preserves member_id,
    # interval_seconds, enabled, last_ping_at, and last_stall_check_at for
    # every row. Safe under FK enforcement: monitor_config is a child table.
...
def downgrade() -> None:
    # Restore the pre-0006 schema; episode state is legitimately reset because
    # it was runtime-ephemeral (columns backfill to defaults, tables recreate
    # empty).
    with op.batch_alter_table("monitor_config") as batch_op:
        batch_op.add_column(
