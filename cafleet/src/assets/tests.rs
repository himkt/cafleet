//! Regression tests for recoverable asset installation.
...
fn real_sql_trigger_failure_rolls_back_entries_and_preserves_exact_old_timestamp() {
    for old_record in [false, true] {
        ...
        let before = fixture.snapshot();
        let record = fixture.record();
        fixture.conn.execute_batch("CREATE TRIGGER deny_new_assets BEFORE INSERT ON asset_installs WHEN NEW.cafleet_version='new-version' BEGIN SELECT RAISE(ABORT,'actual asset record trigger failure'); END;").unwrap();
        failed(
...
#[test]
fn interrupted_reinstall_recovers_after_another_recovery_failure() {
    for initial_operation in [
        InstallOperation::JournalPersist,
        InstallOperation::IntentPersist,
    ] {
        for recovery_operation in [
            InstallOperation::IntentPersist,
            InstallOperation::JournalPersist,
        ] {
            for fault in [
                InstallFault::Interrupt,
                InstallFault::Fail("recovery failure".into()),
            ] {
                let mut fixture = Fixture::new("codex", true, true);
                fixture.install(&okay).unwrap();
                let skill = fixture.paths.skills_dir.join("cafleet");
                fs::write(skill.join("SKILL.md"), b"local edits at the same version").unwrap();
                fs::write(skill.join("local.txt"), b"keep on rollback").unwrap();
                let before = fixture.snapshot();
                let record = fixture.record();
                let cut = Cell::new(false);
                let result = fixture.install(&|event| {
                    if event.operation == initial_operation
                ...
                assert!(inspect_install(&fixture.paths).unwrap().is_some());
                let cut = Cell::new(false);
                let result = fixture.fresh_recover(&|event| {
                    if event.operation == recovery_operation
                        && event.edge == Edge::After
                        && !cut.replace(true)
                    {
                        return Err(match &fault {
                            InstallFault::Interrupt => InstallFault::Interrupt,
                            InstallFault::Fail(message) => InstallFault::Fail(message.clone()),
                        });
                    }
                    Ok(())
                });
...
#[test]
fn reinstall_failure_after_prepared_restores_local_edits() {
    let mut fixture = Fixture::new("codex", true, true);
    fixture.install(&okay).unwrap();
    fs::write(
        fixture.paths.skills_dir.join("cafleet/SKILL.md"),
        b"local edits",
    )
    .unwrap();
    let before = fixture.snapshot();
    ...
    let cut = Cell::new(false);
    failed(
        fixture.install(&|event| {
            if event.operation == InstallOperation::JournalPersist
                && event.edge == Edge::After
                && event.phase == Some(InstallPhase::Prepared)
                && !cut.replace(true)
            {
                return Err(InstallFault::Fail("reinstall failed".into()));
            }
            Ok(())
        }),
        "reinstall failed",
