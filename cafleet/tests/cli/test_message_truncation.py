...
def _message_payload(message_id, *, sender, recipient, text, type_="unicast"):
    return {
...
def _broadcast_summary_payload(message_id, *, sender, count):
    return [
        {
            "message": {
                "message_id": message_id,
                "owner_member_id": sender,
                "from_member_id": sender,
                "to_member_id": None,
...
def _setup_subcommand(
    monkeypatch, subcommand, fleet_id, member_id, other_member_id, message_id
):
    ...
    if subcommand == "poll":
        monkeypatch.setattr(
            ...
            "poll_messages",
            lambda *_a, **_k: [
                _message_payload(1, sender=11, recipient=member_id, text=LONG_BODY)
            ],
        )
        return (
            [
    ...
    monkeypatch.setattr(
        ...
        lambda *_a, **_k: {
            "message": _message_payload(
                message_id, sender=member_id, recipient=other_member_id, text=LONG_BODY
            )
        },
    )
    return [
...
def test_truncation__poll_show_send_json_output(
    ...
    message = payload[0] if shape == "list" else payload["message"]
...
def test_truncation__non_text_fields_byte_identical_between_default_and_full(
    runner, fleet_id, member_id, other_member_id, message_id, monkeypatch, subcommand
):
    args, shape = _setup_subcommand(
        monkeypatch, subcommand, fleet_id, member_id, other_member_id, message_id
    )
    ...
    if shape == "list":
        default_message = json.loads(default_res.output)[0]
        full_message = json.loads(full_res.output)[0]
    else:
        default_message = json.loads(default_res.output)["message"]
        full_message = json.loads(full_res.output)["message"]
    assert default_message["id"] == full_message["message_id"]
    assert default_message["from"] == full_message["from_member_id"]
    assert default_message["ts"] == full_message["status_timestamp"]


@pytest.mark.parametrize("output_mode", ["text", "json"])
@pytest.mark.parametrize("full", [False, True])
def test_truncation__broadcast_summary_emitted_verbatim(
    runner, fleet_id, member_id, message_id, monkeypatch, output_mode, full
):
    monkeypatch.setattr(
        broker,
        "broadcast_message",
        lambda *_a, **_k: _broadcast_summary_payload(
            message_id, sender=member_id, count=3
        ),
    )
    args = [
        "message",
    ...
    if output_mode == "json":
        ...
        assert payload[0]["message"]["text"] == SUMMARY_TEXT
        # Split recipient/delivery counts preserved in JSON.
        assert payload[0]["recipients"] == 3
        assert payload[0]["delivered"] == 3
        if full:
            assert "message_id" in payload[0]["message"]
        else:
            assert "id" in payload[0]["message"]
    else:
        if full:
...
def test_truncation__poll_list_of_three_messages_each_truncated(
    ...
    monkeypatch.setattr(
        ...
        lambda *_a, **_k: [
            _message_payload(
                f"m-{i}", sender=f"from-{i}", recipient=member_id, text=bodies[i]
            )
            for i in range(3)
        ],
    )
