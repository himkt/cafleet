...
def test_poll_tasks__empty_and_non_empty_shape():
    ...
    assert broker.poll_tasks(idle["member_id"]) == []
...
def test_poll_tasks__only_returns_tasks_for_specified_member():
    sid, sender, recipient = _setup_two_members()
...
def test_get_task__sender_or_recipient_can_read(actor_role):
    ...
    tid = sent["task"]["task_id"]
    # get_task is fleet-scoped (doesn't require the actor member_id);
