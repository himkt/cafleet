...
def test_list_roster__active_shape_required_keys_and_fleet_scope():
    ...
    result = broker.list_roster(sid, include_message_holders=True)
    assert len(result) == 3
    ...
    bare = _create_fleet()
    bare_result = broker.list_roster(bare["fleet_id"], include_message_holders=True)
    ...
    assert "in-other" not in {
        a["name"] for a in broker.list_roster(sid, include_message_holders=True)
    }


def test_list_roster__deregistered_with_or_without_messages():
...
def test_get_member__kind_field(scenario, expected_kind):
    ...
    all_entries = broker.list_roster(sid, include_message_holders=True)
...
def test_list_inbox__filters_broadcast_summary_and_owner_member_id_scope():
...
def test_list_timeline__shape_ordering_and_typed_columns():
    ...
    entry = result[0]
    for key in ("message_id", "origin_message_id", "created_at", "text"):
        assert key in entry
...
def test_list_timeline__filters_broadcast_summary_includes_delivery():
    ...
    # Delivery messages (unicast) ARE present.
    assert len(result) >= 2
