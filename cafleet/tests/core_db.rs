//! Step 3 contract tests: `cafleet::db` — connection opener with mandatory
//! PRAGMAs and URL-scheme validation (SPEC §6.1), the embedded refinery
//! migration chain (SPEC §8), the head-DDL introspection, and the
//! chain-guard snapshot.
//!
//! Expected public API:
//! - `connect(database_url: &str) -> Result<rusqlite::Connection, CafleetError>`
//!   — accepts only the `sqlite` scheme (`sqlite:///<path>`); applies
//!   `PRAGMA foreign_keys=ON` and `PRAGMA busy_timeout=5000` on every
//!   connection it opens
//! - `migrate_to_head(&mut rusqlite::Connection) -> Result<u32, CafleetError>`
//!   — applies the embedded chain, returns the head version (1 at cutover);
//!   idempotent
//! - `MIGRATIONS: include_dir::Dir` — the embedded `V<N>__<slug>.sql` chain

use std::collections::BTreeSet;

use cafleet::db::{MIGRATIONS, connect, migrate_to_head};
