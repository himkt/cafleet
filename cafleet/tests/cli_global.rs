...
use common::{Cli, VERSION, code, stderr, stdout, text};
...
fn the_schema_guard_blocks_every_guarded_group() {
    ...
    for args in [
        ...
        assert!(err.contains("run 'cafleet setup'"), "{err}");
    }
...
fn the_guard_ignores_superseded_rows_at_other_paths() {
    ...
    assert_eq!(stdout(&output), "No fleets found.\n");
}
...
fn a_config_location_variable_rekeys_the_guard_to_the_resolved_path() {
    ...
    cli.migrate();
    cli.seed_asset_row("claude", VERSION);
    let missing = cli.run(&["fleet", "list"]);
    assert_eq!(code(&missing), 1);
    assert!(
        stderr(&missing).contains(NO_INSTALL_ERROR),
        "{}",
        stderr(&missing)
    );
    ...
    let current = cli.run(&["fleet", "list"]);
    assert_eq!(code(&current), 0, "{}", stderr(&current));
    assert_eq!(stdout(&current), "No fleets found.\n");
}
