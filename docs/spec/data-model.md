...
The three minted-id tables (`fleets`, `members`, `tasks`) use `INTEGER PRIMARY
...
|---|---|
| `message poll` | Returns the `input_required` deliveries whose `context_id` equals `--member-id`; any in-fleet caller can poll any in-fleet inbox by id. |
...
| `message cancel` | Sender-only — the caller must equal the task's `from_member_id`. |
