...
- **Has a pane (default path)** — send the backend exit keystroke, then wait for the pane to close (poll every 500 ms, up to a 15.0 s timeout). On tmux the agent is the pane's foreground process, so its exit closes the pane; on herdr the pane hosts a persistent shell that outlives the agent, so the wait is realized as "wait for the coding agent to exit, then close the now-shell-only pane." Pane gone → deregister, header `Member deleted.`. A typical coding-agent exit completes in 1–3 s; operators who need faster escalation pass `--force`. On timeout, the pane buffer tail (last 80 lines) is captured and printed on stderr with a recovery hint, and the command exits **2**.
...
Each tick scans the watched set and, when any agent is due, wakes the monitoring member once with a single-line wake nudge naming each due agent (as `<role> <id> (<name>) [<reasons>]`) and the Director, then advances the `last_ping_at` cadence of each agent due by `interval` or `status:done` so it is not re-flagged next tick (a stall-check-only agent is excluded from that stamp — only its stall baseline is committed). The loop **never** keystrokes a watched pane. The watched-set intervals, the wake-nudge contract, and the `Esc`-safeguard placement are canonical on [Monitoring](../concepts/monitoring.md). Each due agent is logged to stdout as `<iso-ts> due agent <id> (<name>) [<reasons>] -> wake monitor`, so the launching background task's output shows live heartbeat activity.
