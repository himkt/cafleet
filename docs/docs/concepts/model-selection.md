...
The monitor and reviewer are policy exceptions on **every** team spawn,
trigger or not:
...
| Monitor member | The backend's monitor default from the model list's *Monitor and reviewer defaults* table | no |
