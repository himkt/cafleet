...
`asset_installs.py`** with `__all__`: import the package and use attribute
access (`from cafleet import broker` then `broker.send_message(...)`). The
`asset_installs.py` helpers are the deliberate exception — they are imported
directly from the submodule (`from cafleet.broker.asset_installs import …`).
