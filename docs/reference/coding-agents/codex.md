...
The working directory must also be trusted before spawning — see [Quickstart § Trust the working directory](../../quickstart.md#trust-the-working-directory) for the spawn-time stall this prevents. Codex reads trust entries from `~/.codex/config.toml`; the path is the absolute working directory the member panes run in:
