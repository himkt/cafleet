...
| Agent-card render slim | `output.render_agent` projects `agent_card_json` to the minimum-required fields by default (`agent_id`, `name`, `description` truncated, `status`, `coding_agent`); the full card is returned on `--full`. The storage-side `agent_card_json` blob is unchanged — render-side projection captures most of the win. |
