...
**Manual entry-point**: `tmux.send_poll_trigger` survives as the helper for the **manual** poll-nudge path only — the sole caller is `cafleet member ping` (the Director-only manual nudge subcommand, which converts a `False` return to exit 1 so an operator or monitoring loop sees the failure). Auto-fire on every `cafleet message send` uses `tmux.send_inline_preview`; the `member ping` path keeps the "type the poll command + Enter" behavior because that primitive is exactly what an operator wants when the recipient missed an inline preview and needs to drain whatever has accumulated.
