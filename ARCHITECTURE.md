...
**Write-path authorization mirrors the read path**: `cafleet member send-input` — a safe `tmux send-keys` wrapper for answering an `AskUserQuestion` prompt — and `cafleet member safe-exec` — the permission-aware bash dispatch that keystrokes `! <cmd>` via the `!` shortcut in a member's pane — both reuse the exact `member capture` authorization boundary (`placement.director_agent_id == --agent-id`, non-null `tmux_pane_id`, placement row present). `member send-input` accepts exactly one of `--choice {1,2,3}` (sends the matching digit key, AskUserQuestion-only) or `--freetext "<text>"` (sends `4`, the literal text via tmux's `-l` flag, then `Enter` — AskUserQuestion-only since `4` selects the "Type something" option). `member safe-exec` accepts only `--bash "<cmd>"` (sends literal `! <cmd>` then `Enter` after the permission decision allows the command). Newlines are rejected for `--freetext` and `--bash` at both the CLI layer and the helper, so each call is exactly one prompt submission. The helper never invokes a shell (`subprocess.run([...], shell=False)`), so shell meta, backticks, `$VAR`, and multi-byte characters pass through as literal input.
