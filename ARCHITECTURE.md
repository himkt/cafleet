...
| `coding_agent/` | `cafleet/src/cafleet/coding_agent/` | Coding-agent backend subpackage: `CodingAgent` `@runtime_checkable` Protocol (`base.py`), `ClaudeCodeAgent` (`claude.py`), `CodexAgent` (`codex.py`), and the `CODING_AGENTS: dict[str, CodingAgent]` registry exposed from `__init__.py`. `cli.member_create` resolves `CODING_AGENTS[coding_agent].build_spawn_argv(...)` instead of an inline `if/else` per backend, and `click.Choice` for `--coding-agent` is computed from `list(CODING_AGENTS.keys())`. |
