...
| `base_dir.py` | `cafleet/src/cafleet/` | Authoritative resolver for the `${BASE}` output-root used by every CAFleet scratch / audit / figure write. Owns the resolution state machine: the no-positional branch (`resolved` from CWD inference / anchor, `needs-user-input` for `$HOME`-rooted CWDs, fatal-error on schema mismatch) and the positional `TASK_NAME` branch (task-scope resolution under the inferred repo root, auto-mkdir of the task folder, inline anchor write at `<task-folder>/.cafleet-base-dir.json` with `source: "task-scope"`; absolute-path arguments either match a known `researches/<slug>/` or `design-docs/<NNNNNNN>-<slug>/` ancestor or fall through to the `<unset>` sentinel). The on-disk anchor schema (`version=1`, `base`, `source`, `resolved_at`) accepts `cwd-inference` / `askuserquestion` / `task-scope` in the `source` field. Exposed via the `cafleet base-dir` CLI subgroup. |
