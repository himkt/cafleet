...
| `cli.py` | `cafleet/src/cafleet/` | Unified `cafleet` console script: click group with `db` (Alembic schema management), `session` (session CRUD), `agent` (registry: `register` / `deregister` / `list` / `show`), `message` (broker: `send` / `broadcast` / `poll` / `ack` / `cancel` / `show`), and `member` (lifecycle: `create` / `delete` / `list` / `capture` / `send-input` / `exec` / `safe-exec`) subgroups. Also exposes `cafleet server [--host <addr>] [--port <int>]` and `cafleet doctor` as top-level meta-command exceptions. Calls `broker` directly. |
| `permissions.py` | `cafleet/src/cafleet/` | Permission-aware shell dispatch utilities. Exposes `discover_settings_paths`, `load_bash_patterns`, `match`, `decide` for `cafleet member safe-exec`. Re-reads `.claude/settings*.json` on every call; zero coupling to `broker` / `Settings` / `cli`. |
...
| `member send-input` | `broker.get_agent()` → authorization check + `tmux.send_choice_key` / `tmux.send_freetext_and_submit`. Director-side workflow is AskUserQuestion-delegated for `--choice`/`--freetext` (AskUserQuestion frame). See [`skills/cafleet/SKILL.md`](skills/cafleet/SKILL.md) "Answer a member's AskUserQuestion prompt" for the canonical workflow. |
| `member exec` | `broker.get_agent()` → authorization check + `tmux.send_bash_command` keystrokes `! <cmd>` + Enter directly into the member's normal Claude Code prompt. NO internal permission decision — the outer Bash hook layer (Claude Code's `permissions.ask` on the Director's invocation) is the operator-confirmation surface. See [`skills/cafleet/SKILL.md`](skills/cafleet/SKILL.md) "Routing Bash via the Director" and the "Permission-aware shell dispatch" section below. |
| `member safe-exec` | `broker.get_agent()` → authorization check + `permissions.decide()` over the union of `Bash(...)` patterns from three settings.json files. On allow, `tmux.send_bash_command` keystrokes `! <cmd>` + Enter into the member's normal Claude Code prompt; deny exits 2; ask exits 3. See [`skills/cafleet/SKILL.md`](skills/cafleet/SKILL.md) "Routing Bash via the Director" and the "Permission-aware shell dispatch" section below. |
...
**Commands**: `member create`, `member delete`, `member list`, `member capture`, `member send-input`, `member exec`, `member safe-exec`. All require `--agent-id` (the Director's ID). The tmux helper module (`cafleet/src/cafleet/tmux.py`) isolates all subprocess interaction with tmux. Primitives for pane lifecycle inspection and forced teardown — `pane_exists`, `kill_pane`, and `wait_for_pane_gone` — live here so the CLI never calls tmux directly. Director-side usage of `member send-input --choice` / `--freetext` is AskUserQuestion-delegated (capture → Director-side `AskUserQuestion` → direct Bash invocation of the resolved command); see [`skills/cafleet/SKILL.md`](skills/cafleet/SKILL.md) "Answer a member's AskUserQuestion prompt" for the canonical three-beat workflow and pane-shapes table. `member exec CMD` and `member safe-exec CMD` are the bash-routing modes — see § Permission-aware shell dispatch below.

**Write-path authorization mirrors the read path**: `cafleet member send-input` — a safe `tmux send-keys` wrapper for answering an `AskUserQuestion` prompt — and the bash-dispatch siblings `cafleet member exec` and `cafleet member safe-exec` — which keystroke `! CMD` via the `!` shortcut in a member's pane — all reuse the exact `member capture` authorization boundary (`placement.director_agent_id == --agent-id`, non-null `tmux_pane_id`, placement row present). `member send-input` accepts exactly one of `--choice {1,2,3}` (sends the matching digit key, AskUserQuestion-only) or `--freetext "<text>"` (sends `4`, the literal text via tmux's `-l` flag, then `Enter` — AskUserQuestion-only since `4` selects the "Type something" option). `member exec` and `member safe-exec` each accept a single positional `CMD` argument (no `--bash` flag) and send literal `! CMD` then `Enter` once the dispatch is approved (immediately for `exec`; only on permission-decision allow for `safe-exec`). Newlines in `--freetext` and `CMD` are rejected at both the CLI layer and the helper, and `CMD` is rejected if empty after `.strip()`, so each call is exactly one prompt submission. The helper never invokes a shell (`subprocess.run([...], shell=False)`), so shell meta, backticks, `$VAR`, and multi-byte characters pass through as literal input.
...
The bash-via-Director protocol is the **fallback** for the harness deny-list: dontAsk does not auto-resolve everything — destructive operations such as `git push` and `rm -rf` are still rejected at the Claude Code harness layer. When a member's Bash invocation is denied, the member auto-routes by sending a plain CAFleet message to its Director, and the Director dispatches the command into the member's pane via `cafleet member safe-exec CMD` (silent fast-path) or `cafleet member exec CMD` (operator-prompted fallback), each of which keystrokes literal `! CMD` + `Enter` to trigger Claude Code's `!` CLI shortcut on the receiving side. Members must first reconsider whether the rejected command is correct and necessary — most denials are caused by a wrong command, not a missing privilege. See [`skills/cafleet/SKILL.md`](skills/cafleet/SKILL.md) § Routing Bash via the Director for the full convention, including the smart-routing rule.

## Permission-aware shell dispatch

The Director has two sibling subcommands for bash routing, both taking the command as a single positional `CMD` argument:

- **`cafleet member exec CMD`** — bare dispatcher. NO internal permission decision. Keystrokes `! CMD` + `Enter` into the member's pane after authorization and pre-flight validation. The outer Bash hook layer (Claude Code's `permissions.ask` on the Director's invocation) is the operator-confirmation surface.
- **`cafleet member safe-exec CMD`** — permission-aware dispatcher. Every invocation re-reads three `settings.json` files and matches the inner CMD against the operator's existing `Bash(...)` allow / deny patterns. Allow → keystroke (exit 0). Deny → reject with the matched pattern, file, and offending command on stderr (exit 2). Ask (no allow pattern matches) → list the three searched files plus a suggested `Bash(...)` pattern on stderr (exit 3).

**Smart-routing rule (Director-side):** when the bash-via-Director protocol fires, the Director picks between `safe-exec` and `exec` using a check-then-fallback rule: try `safe-exec` first; on exit 0 the command was already dispatched silently, on exit 2 relay the deny block to the operator, on exit 3 fall back to `exec` (which surfaces an outer `permissions.ask` prompt for the operator). This routes routine pre-rule'd commands through the silent fast-path while everything else gets per-call operator confirmation. The operator does not have to maintain a comprehensive allow list — unknown commands surface as outer-layer prompts via `exec`.

**Three-file discovery** (in matcher-precedence order, no caching at any layer; safe-exec only):
...
Each path is consulted on every call. Missing files are treated as empty `{"permissions": {"allow": [], "deny": []}}` documents. Malformed JSON raises with the file path. A `permissions` key that lacks `allow` / `deny` is treated as empty. Non-`Bash(...)` entries (`Read(...)`, `WebFetch(...)`, …) are silently filtered out. Allow lists and deny lists are concatenated (unioned) across all three files; deny patterns are checked first, so deny wins on any conflict.

**Tri-state outcome (safe-exec)**:
...
| **ask** | No allow pattern matches. CMD is NOT dispatched. Stderr lists the three searched files (with the resolved user path) and a suggested `Bash(...)` pattern the operator can add. | 3 |

Exit code 2 for deny aligns with Claude Code PreToolUse hook convention (exit 2 stops a tool call). Exit code 3 for ask is unique to `safe-exec`, signaling operator intervention required (edit `settings.json` and re-run, OR fall back to `exec` per smart-routing). `exec` never emits exit 3 — it has no internal ask path. Exit code 1 is reserved for runtime / IO / authorization failures (both subcommands).

**Glob matcher (safe-exec)**: Patterns are converted to anchored regex via `re.fullmatch`. The trailing form determines word-boundary semantics — `Bash(git status:*)` and `Bash(git status *)` match `git status` and `git status --short` but not `git statuses`; `Bash(gitstatus*)` matches any string starting with `gitstatus`; `Bash(*)` matches anything. Interior stars (`Bash(* install)`, `Bash(git * main)`) match arbitrary text at that position only — they do NOT add a trailing wildcard. `re.escape` protects every non-`*` character.

**JSON output**: `cafleet --json member safe-exec CMD` emits five keys for all three outcomes — `outcome`, `matched_pattern`, `matched_file`, `offending_substring`, `searched_files` — letting consumers parse decisions by key. `cafleet --json member exec CMD` emits four keys mirroring the legacy `send-input --bash` shape — `member_agent_id`, `pane_id`, `action` (literal `"bash"`), `value` — since `exec` carries no decision metadata.

**Out of scope (deferred Claude Code parity features)**: process-wrapper stripping (`timeout`, `time`, `nice`, `nohup`, `stdbuf`, `xargs`); read-only command auto-allow (`ls`, `cat`, `grep`, `find`, `wc`, ...); exec-wrapper auto-prompt (`watch`, `setsid`, `ionice`, `flock`); compound-command split-and-match (pipes, `&&`, `;`, `$(...)`, backticks). The matcher operates on the inner CMD as one opaque string. Compound-command rejection happens at the outer Bash hook layer (the `validate_bash.py` project hook plus Claude Code's permission system on the Director's `cafleet ...` invocation), never inside `safe-exec` or `exec`.
