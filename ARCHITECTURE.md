...
**Bash routing**: Members spawn with `--no-bash` (default for `cafleet member create`), which appends `--disallowedTools "Bash"` to the spawn argv. When a member needs a shell command, it sends a plain CAFleet message to its Director asking for the command. The Director (or operator at the Director's pane) decides whether to fulfill the request and responds by sending `! <command>` keystrokes into the member's pane via `cafleet member send-input`. Claude Code's `!` CLI shortcut handles execution natively — the output appears in the member's pane prompt context for the model to read. No new broker primitives, no JSON envelopes, no separate helper subcommand — just the existing message-passing + tmux-keystroke infrastructure. Multiple concurrent shell-command requests still serialize through the broker's existing message queue because the Director processes each `cafleet message poll`-returned request one at a time, dispatches the `! cmd` keystroke via `cafleet member send-input`, and then moves to the next request. No new queueing primitive is required, but the processing order follows the broker's current polling behavior rather than a FIFO guarantee.
