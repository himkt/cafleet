...
| `cli.py` | `cafleet/src/cafleet/` | Unified `cafleet` console script: click group with `db` (Alembic schema management), `session` (session CRUD), `agent` (registry: `register` / `deregister` / `list` / `show`), `message` (broker: `send` / `broadcast` / `poll` / `ack` / `cancel` / `show`), `member` (lifecycle: `create` / `delete` / `list` / `capture` / `send-input` / `exec`), and `base-dir` (filesystem-only: `resolve` / `record`) subgroups. Also exposes `cafleet server [--host <addr>] [--port <int>]` and `cafleet doctor` as top-level meta-command exceptions. Calls `broker` directly. |
| `base_dir.py` | `cafleet/src/cafleet/` | Authoritative resolver for the `${BASE}` output-root used by every CAFleet scratch / audit / figure write. Owns the four-status resolution state machine (`resolved` / `unset` / `needs-user-input` / fatal-error), the on-disk anchor schema (`${BASE}/.cafleet-base-dir.json` with `version=1`, `base`, `source`, `resolved_at`), and the `<unset>` sentinel returned on the absolute-path branch. Exposed via the `cafleet base-dir` CLI subgroup. |
...
- `cafleet base-dir resolve [--path <abs-or-rel-arg>] [--json]` — probe-only resolution. Emits one of four statuses: `resolved` (BASE determined from CWD inference or an existing anchor), `unset` (caller passed an absolute-path argument — `${BASE}` is intentionally undefined and consumers must guard-skip BASE-derived writes), `needs-user-input` (CWD is `$HOME` or under `$HOME/.claude` and no usable anchor exists — Claude drives `AskUserQuestion` on the returned candidates), or a non-zero exit with a standardized error (anchor schema / version mismatch). Never falls back to `/tmp` silently.
