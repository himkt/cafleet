...
The token-budget regression suite under `tests/token_budget/` (Surface 13) checks character counts on representative outputs against checked-in baselines so future drift is caught at PR time. `tests/token_budget/scenarios/idle_3_member_10_minute.py` is the manually-runnable measurement scenario for the per-tick cost of the Director monitoring commands.
